"""Unit tests for the pure logic in ``deploy/registry/build_and_push_images.py``.
...
_SCRIPT = _REPO_ROOT / "deploy" / "registry" / "build_and_push_images.py"
...
def test_target_ref_prefixes_portable_ref() -> None:
    assert bp.target_ref("10.0.0.1:5011", "xrlenv-seta-env/0:main") == (
        "10.0.0.1:5011/xrlenv-seta-env/0:main"
    )


def test_target_ref_is_idempotent_when_already_prefixed() -> None:
    already = "10.0.0.1:5011/xrlenv-seta-env/0:main"
    assert bp.target_ref("10.0.0.1:5011", already) == already
...
def test_split_ref_host_repo_tag_with_port() -> None:
    assert bp.split_ref("10.0.0.1:5011/xrlenv-seta-env/0:main") == (
        "10.0.0.1:5011", "xrlenv-seta-env/0", "main",
    )
