#!/usr/bin/env python3
"""Build a shared harbor task cache for the FrontierSWE benchmark.

Self-contained pipeline, mirroring ``deep_swe/build_cache.py`` in shape
(``--stage`` driven, idempotent), retargeted at FrontierSWE — a
**harbor-format task corpus** (``Proximal-Labs/frontier-swe``) whose tasks live
directly in-git under ``tasks/<id>/``. Each task unpacks to
``task.toml`` + ``instruction.md`` + ``environment/Dockerfile`` +
``tests/{test.sh,compute_reward.py}`` + (for the oracle-gateable subset)
``solution/solve.sh`` — the exact harbor filesystem contract
``TaskConfig(path=…)`` consumes.

Stages
------
    populate  Materialize the task dirs into ``<dest>/frontier-swe/<id>/`` and
              normalize each ``task.toml``. Source = ``git`` (shallow-clone
              ``Proximal-Labs/frontier-swe`` and copy ``tasks/<id>/`` into the
              shard; needs network + ``git``). ``FRONTIER_SWE_REPO_URL`` may point
              at a **local clone** for an offline re-populate.
    patch     Overlay curated ``patches/<id>/`` full-file fixes onto the extracted
              tasks (start EMPTY — FrontierSWE's oracle verifiers are deterministic
              local computations, so the unpinned-dep-drift risk that motivates
              tb2.1's pins is low here; the hook stays for when the oracle sweep
              surfaces broken content).
    all       populate (only if missing) then patch. The default.

Oracle-gateable subset
----------------------
FrontierSWE is a **live leaderboard**, so upstream withholds the reference
solution for several tasks (they ship ``tests/`` + ``task.toml`` but no
``solution/solve.sh``). A task with no reference solution **cannot be oracle-
gated** — there is nothing for harbor's OracleAgent to apply. This script
populates the FULL corpus faithfully (every ``tasks/<id>/`` with a ``task.toml``)
and reports how many are oracle-gateable (carry ``solution/solve.sh``); the sweep
+ ``run_full_sweep.sh`` discover on ``solution/solve.sh``, so the withheld-
solution tasks are simply never enumerated for the gate. See STATUS.md for the
per-task disposition (GPU / solution-withheld / green).

Image cache (separate from this task-dir cache)
-----------------------------------------------
FrontierSWE ships **prebuilt per-task images on public GHCR**
(``[environment] docker_image = ghcr.io/proximal-labs/frontier-swe/<id>:<tag>``,
anonymous-pullable). This script populates only the *task-dir* cache; the images
are warmed separately via ``build_plan_gen.py`` (a ``type: registry`` plan) +
``xrlenv build apply``. Do NOT re-push them into the private registry (public
GHCR needs no new infra — the deep_swe direct-registry decision).

How xrlenv consumers pick it up
-------------------------------
harbor resolves a task by local *path* (``TaskConfig(path=…)``), used as-is, so
the normalized ``task.toml`` this script writes is authoritative. Point every
consumer's ``XRLENV_BENCHMARK_CACHE`` at the shared root this script writes; tasks
land at ``<root>/frontier-swe/<id>/`` beside the tb2.1 / deep-swe shards.

Env overrides (populate):
    FRONTIER_SWE_REPO_URL   git remote (or a local clone path)
                            (default https://github.com/Proximal-Labs/frontier-swe)
    FRONTIER_SWE_REPO_REF   git ref/branch   (default main)
    FRONTIER_SWE_SHARD      cache shard name (default frontier-swe)
"""
...
REPO_URL = os.environ.get(
    "FRONTIER_SWE_REPO_URL", "https://github.com/Proximal-Labs/frontier-swe"
)
REPO_REF = os.environ.get("FRONTIER_SWE_REPO_REF", "main")
# Shard subdir name == image namespace root (one name, double duty — same
# convention as the deep-swe / terminalworld shards). The name every consumer's
# shard-scan sees.
SHARD = os.environ.get("FRONTIER_SWE_SHARD", "frontier-swe")
...
# extraction+normalization, so the fixes survive re-populate. Starts empty.
...
# ...and is oracle-gateable iff it ALSO ships a reference solution.
...
def normalize_task_toml_text(text: str) -> tuple[str, bool]:
    ...
    (FrontierSWE uses ``_mb`` only — kept defensively so an upstream schema
    change doesn't silently break the load).
    """
...
def _copy_tasks(tasks_root: Path, shard_dir: Path) -> tuple[int, int]:
    ...
    if not tasks_root.is_dir():
        raise SystemExit(
            ...
            f"frontier-swe repo layout? (tasks/<id>/task.toml)",
        )
    ...
    for i, src in enumerate(task_dirs, 1):
        ...
        if _normalize_task_toml(dest / _TASK_ANCHOR):
            normalized += 1
        if i % 5 == 0 or i == len(task_dirs):
            print(f"   [{i}/{len(task_dirs)}] {src.name}", file=sys.stderr)
    return moved, normalized


def populate_git(shard_dir: Path) -> tuple[int, int]:
    """Shallow-clone the frontier-swe repo and copy its ``tasks/`` into the shard.

    ``FRONTIER_SWE_REPO_URL`` may be a **local** path (an existing clone) — ``git
    clone`` handles a local source natively, so an offline re-populate needs no
    network.
    """
    with tempfile.TemporaryDirectory(prefix="frontier-swe-clone-") as tmp:
        print(
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="build_cache",
        description=(
            "Materialize the FrontierSWE task corpus into a shared harbor cache. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"{total - gateable} solution-withheld)"
