"""Unit tests for the DeepSWE cache builder's pure/filesystem logic.

Network-free: the ``populate`` git/hf download is not exercised here (covered by
running ``--stage populate`` live); these cover ``normalize_task_toml_text``, the
idempotent ``_copy_tasks``, and the ``patches/`` overlay.
"""
from __future__ import annotations

from pathlib import Path

from xrlenv_plugins.benchmarks.deep_swe.build_cache import (
    _apply_patch,
    _copy_tasks,
    _count_tasks,
    apply_all_patches,
...
def test_normalize_strips_deprecated_when_canonical_present() -> None:
    ...
    assert 'docker_image = "ecr/x:tag"' in out  # untouched


def test_normalize_strips_in_verifier_environment_section() -> None:
    text = (
        "[environment]\n"
        "memory_mb = 8192\n"
        "\n"
    ...
    out, changed = normalize_task_toml_text(text)
    assert changed
    # the deprecated line inside [verifier.environment] is dropped ...
    assert out.count("memory =") == 0
    # ... and both canonical _mb lines survive
    assert out.count("memory_mb = 8192") == 2
...
def test_normalize_noop_when_only_deprecated() -> None:
    # No conflict (harbor/pier migrate a lone ``memory`` into ``memory_mb``), so
    # we leave it — dropping it would be lossy.
    text = '[environment]\nmemory = "8G"\n'
    out, changed = normalize_task_toml_text(text)
    assert not changed
    assert out == text
...
# ── _copy_tasks (idempotent) + normalize-on-copy ──────────────────────────────


def _make_src_task(root: Path, name: str, task_toml: str) -> Path:
    d = root / name
...
def test_apply_all_patches_skips_absent_task(tmp_path: Path, monkeypatch) -> None:
    import xrlenv_plugins.benchmarks.deep_swe.build_cache as bc
    ...
    # a patch for a present task ...
    (patches / "present").mkdir(parents=True)
    (patches / "present" / "note.txt").write_text("x\n")
    # ... and one for an absent task (skipped, not an error)
