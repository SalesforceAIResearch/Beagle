"""Generic pier/harbor multi-service compose helpers — pure (``yaml`` + stdlib only).
...
def subdir_build_services(compose: dict[str, Any]) -> dict[str, str]:
    """``iter_build_services`` restricted to *sub-directory* contexts — the
    services that need a distinct build entry beyond the task's own ``<id>``
    image (whose context is ``.``). Empty for the common case where every
    ``build:`` service shares the environment dir."""
    return {n: c for n, c in iter_build_services(compose).items() if c != MAIN_CONTEXT}
...
def default_image_refs(
    ...
    *,
    ...
    - context ``.`` → the task's own image, ``main_ref`` if given else
      ``<namespace>/<task_id>:<tag>`` (matches today's single-service ref);
    - sub-dir context → ``<namespace>/<task_id>-<service>:<tag>``.
    ...
    refs: dict[str, str] = {}
    for name, ctx in iter_build_services(compose).items():
        if ctx == MAIN_CONTEXT:
...
def assemble_project(
    ...
    *,
    ...
    it from the repinned main ref via :func:`harbor.compose.registry_namespace_and_tag`);
    passing ``None`` there raises ``ValueError`` rather than emitting a ref that won't
    match what was pushed. A task with **no** sub-dir builds needs no ``namespace`` —
    every ``build:``/local-tag service resolves to ``main_ref``.
    """
