...
xrlenv build apply \
    ...
    --operator-token write_jW3bNk...

# Or pass it as an Authorization header in any HTTP client:
curl -H "Authorization: Bearer write_jW3bNk..." \
...
1. Issue a viewer token once:
   ...
   ```bash
   xrlenv tokens issue viewer
   # read_jW3bNkQzPvXmTsLyAfGhRuCwEdBiOa
   ```
...
3. Each teammate navigates to `http://control-plane:8080/`, pastes
   `read_jW3bNkQzPvXmTsLyAfGhRuCwEdBiOa` on the sign-in page, and gains
...
xrlenv tokens list
# → viewer    active   token_id=a3f9c1d82b47 digest_hint=a3f9c1 ...
xrlenv tokens revoke a3f9c1                 # revoke by the 6-char digest_hint
# or:
xrlenv tokens revoke a3f9c1d82b47           # revoke by the full 12-char token_id
```
