...
# on /fsx silently falls back to the root disk — the "data-root was a
...
#   sudo bash deploy/node/set_docker_data_root.sh
# Override the target if your EBS volume is mounted elsewhere:
#   sudo DATA_ROOT=/mnt/ebs/docker/data-root bash deploy/node/set_docker_data_root.sh
