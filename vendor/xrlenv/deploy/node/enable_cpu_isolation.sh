#!/usr/bin/env bash
# enable_cpu_isolation.sh — make THIS node CPU-isolation-capable (opt-in).
#
# CPU isolation confines unpinned runc
...
# ``isolation_capable=true`` only after a real self-test — gated (v1) on the
...
#
# WHY THIS SCRIPT (and not the agent) SETS THINGS UP. The node agent
# runs as a NON-ROOT user (``User=xrlenv``); it cannot ``mkdir`` / write
...
#
# REBOOT PERSISTENCE: cgroups don't survive a node reboot, so
# ``xrlenv-shared`` + its delegation would be lost. This script installs
# deploy/node/setup_shared_cpuset.sh locally + enables a systemd oneshot
...
# CPU isolation opt-in (deploy/node/enable_cpu_isolation.sh). Loaded by the
...
# ── 5. REAL container probe (root) — the capability gate ─────────────────────
...
# ── 6. create + DELEGATE xrlenv-shared to the (non-root) agent user ──────────
# Install the setup script LOCALLY (so the boot unit below doesn't depend on the
# shared /fsx mount being up), then run it to (re)create + delegate the cgroup.
...
# re-runs the setup After=docker Before=xrlenv-node on each boot.
