...
# the control-plane box. This is "Phase B" of notes/image-cache-improvements-plan.md:
# a `docker push`-able registry that holds images we build ourselves from
# Dockerfiles (e.g. camel-ai/seta-env tasks), backed by FSx.
#
# It is the SIBLING of run-registry-proxy.sh — keep the two straight:
#   - run-registry-proxy.sh → :5010, PROXY (pull-through cache of docker.io).
...
#   XRLENV_PRIVATE_REGISTRY_STORAGE      FSx blob-store path
#                                      (default /path/to/data$USER/xrlenv-registry/private)
...
# run-registry-proxy.sh — the .env is hand-maintained, so parse rather than
...
REGISTRY_STORAGE="${REGISTRY_STORAGE:-/path/to/data${USER}/xrlenv-registry/private}"
...
# Storage dir on FSx + shared-fs guard (symmetric to run-registry-proxy.sh: we
...
echo "       sudo PRIVATE_REGISTRY=<this-host-ip>:${REGISTRY_PORT} bash scripts/configure_docker_registry.sh --restart"
echo "  2. Build + push a plan's images (fan out across CPU instances):"
echo "       sbatch slurm_scripts/build_and_push_images.sh"
echo "     or a single shard locally:"
echo "       python scripts/build_and_push_images.py --plan <build_plan.yaml> \\"
