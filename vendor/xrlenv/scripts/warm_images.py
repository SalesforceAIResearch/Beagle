...
Usage:
  python3 scripts/warm_images.py <build_plan.yaml> \
...
def _default_store() -> str:
    p = os.environ.get("XRLENV_REGISTRY_STORAGE")
    if p:
        return p
    return f"/path/to/data{os.environ.get('USER', '')}/xrlenv-registry/proxy"
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("--store-path", default=None,
                    help="FSx registry blob-store root used to SKIP already-cached "
                         "blobs (default: $XRLENV_REGISTRY_STORAGE or "
    ...
    if not args.no_skip:
        ...
        if not os.path.isdir(store):
            print(f"==> note: store path {store!r} not found — skip-existing "
                  ...
                  f"or set XRLENV_REGISTRY_STORAGE to enable resume.", flush=True)
            store = None
