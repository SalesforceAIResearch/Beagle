...
This is the "Phase B" build-once-push glue from
``notes/image-cache-improvements-plan.md``: instead of every worker rebuilding a
...
It is the SIBLING of ``scripts/warm_images.py``: warm_images fills the *proxy*
...
shared FSx. So shard the plan and run one shard per host:
  ...
  * Single host, everything:
      python scripts/build_and_push_images.py --plan build_plan.yaml \\
          --registry internal-ip:5011

  * One shard of N (run N copies, one per CPU instance):
      python scripts/build_and_push_images.py --plan build_plan.yaml \\
          --registry internal-ip:5011 --shard-index 3 --num-shards 16

  * Under Slurm (one shard per node): shard index/count are auto-read from
      $SLURM_PROCID / $SLURM_NTASKS (or $SLURM_ARRAY_TASK_ID /
      $SLURM_ARRAY_TASK_COUNT for a job array) — see the ready-to-run batch job
      slurm_scripts/build_and_push_images.sh.
...
# from `xrlenv build apply`'s notion of a plan (CLAUDE.md: don't reinvent).
...
def target_ref(registry: str, image_ref: str) -> str:
    ...
    pushed/pulled ref is ``internal-ip:5011/xrlenv-seta-env/0:main``. Idempotent:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--registry", required=True,
                   help="Private registry host:port to push to, e.g. internal-ip:5011.")
