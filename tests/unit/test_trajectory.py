...
def test_subagent_usage_stays_main_only_in_final_metrics(tmp_path) -> None:
    """A `subagent_usage` event carries the sub-agent's own turns+tokens as
    SELF-DESCRIBING flat fields (not a `usage` dict), so the reducer must NOT fold it
    into the main-agent total — `final_metrics` stays main-only (preserving the shape
    downstream consumers depend on) and the event adds no spurious step. The
    sub-agent split is summed separately into `main_plus_sub_agents_*` by beagle."""
    stream = "\n".join([
        '{"type":"tool_use_start","index":0,"toolId":"c1","toolName":"agent"}',
        '{"type":"tool_use_delta","index":0,"partialJson":"{\\"prompt\\":\\"help\\"}"}',
        '{"type":"tool_output","id":"c1","output":"delegated result","isError":false}',
        '{"type":"usage","usage":{"inputTokens":1000,"outputTokens":100,"cacheTokens":800}}',
        '{"type":"turn_complete","turn":0,"stopReason":"end_turn"}',
        '{"type":"subagent_usage","worker":"helper","turns":35,"inputTokens":5000,"outputTokens":300,"cacheTokens":4000}',
        '{"type":"turn_done"}',
    ])
    (tmp_path / "monet.stream.jsonl").write_text(stream)
    path = write_trajectory_json(
        tmp_path, trajectory_format="monet-stream-json", instruction="x",
        agent_name="monet", agent_version="v", model_name="m",
    )
    assert path is not None
    d = json.loads(path.read_text())
    fm = d["final_metrics"]
    # MAIN-ONLY — the sub-agent's 5000/300/4000 is NOT folded in.
    ...
    assert [s["source"] for s in d["steps"]] == ["user", "agent"]

    # The full accounting (main-only + delegated split + true main_plus_sub_agents_*)
    # is recorded in the usage_with_subagents.json sidecar for downstream consumers.
    side = json.loads((tmp_path / "usage_with_subagents.json").read_text())
    assert side["main_only"]["total_prompt_tokens"] == 1000
    assert side["sub_agents"] == {"total_steps": 35, "total_prompt_tokens": 5000,
                                  "total_cached_tokens": 4000, "total_completion_tokens": 300}
    assert side["main_plus_sub_agents_total_steps"] == 1 + 35
    assert side["main_plus_sub_agents_total_prompt_tokens"] == 1000 + 5000
    assert side["main_plus_sub_agents_total_cached_tokens"] == 800 + 4000
    assert side["main_plus_sub_agents_total_completion_tokens"] == 100 + 300
