...
def parse_monet_subagent_usage(monet_stdout: str) -> tuple[int, Usage]:
    """Sum the DELEGATED (sub-agent) turns + token usage from ``subagent_usage`` events.

    The agent tool emits one such event per resolved sub-agent, carrying its own turn
    count and ``{inputTokens, outputTokens, cacheTokens}`` as SELF-DESCRIBING flat fields
    (deliberately not a ``usage`` dict) so :func:`parse_monet_usage` / the ATIF reducer keep
    the main-agent totals MAIN-ONLY. This is the separate delegated split; add it to the
    main-only totals for the true ``main_plus_sub_agents_*`` count."""
    turns = 0
    total = Usage()
    for o in _iter_json_lines(monet_stdout):
        if o.get("type") == "subagent_usage":
            turns += int(o.get("turns") or 0)
            total = usage_add(total, _monet_event_usage(o))  # o has inputTokens/outputTokens/cacheTokens
    return turns, total
