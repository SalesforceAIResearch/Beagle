...
def analyze_digest_block(
    ...
    *,
    ...
    # nothing on semantic failures. DARWINX_EVOLVE_TRACE_QC_LLM=1 turns it on; the config
    ...
    llm_client = None
    qc_config = config
    if os.environ.get("DARWINX_EVOLVE_TRACE_QC_LLM", "0").strip().lower() in ("1", "true", "yes", "on"):
        qc_config = os.environ.get("DARWINX_EVOLVE_TRACE_QC_CONFIG", "monet")
        try:
            from dx_trace.llm import OpenAIClient
    ...
    # Strict no-op unless DARWINX_GATE_TEACHER_SUPERVISION=1 AND a cached teacher trace
    ...
    # OPTIONAL GRPO-style self-rollout contrast (env-gated DARWINX_GATE_SELF_CONTRAST=1;
...
def _write_messages_sidecar(jsonl_path: Path) -> Path | None:
    ...
    try:
        ...
        from dx_trace.normalizer import load
