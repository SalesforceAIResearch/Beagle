...
# Per-eval scratch (the ``_iso/<uuid>`` run dirs and the emitted config handshake) lands
# under this root. Defaulting it inside the repo writes scratch into source, which is wrong
# anywhere the driver is embedded as a package rather than run from its own checkout — the
# scratch then pollutes someone else's tree. An embedder points this at the run directory.
# Read at import because the value is baked into a dataclass default below, so a host must
# set it before importing this module.
DEFAULT_RESULTS_ROOT = (
    Path(os.environ["DARWINX_EVAL_RESULTS_ROOT"])
    if os.environ.get("DARWINX_EVAL_RESULTS_ROOT", "").strip()
...
# exp_05 lineage. We point at ``your-org/monet_code`` — an OWNED
# fork of that lineage (forked from ``upstream/monet_code``), so
...
# push target. (The earlier vendored pin ``an-earlier/monet_code@eval``
...
# Override for YOUR fork via ``DARWINX_EVOLVE_MONET_REPO_URL`` (e.g. in .env): the
...
# Express-provider lineage). Defaults to your configured fork.

DEFAULT_MONET_REPO_URL = (
    os.environ.get("DARWINX_EVOLVE_MONET_REPO_URL", "").strip()
    or "https://github.com/your-org/monet_code.git"
)
...
_RUNTIME_ENV = "DARWINX_EVOLVE_EVAL_RUNTIME"
...
def _resolve_runtime_kwargs(runtime_block: Any) -> dict[str, Any]:
    ...
    Precedence: ``$DARWINX_EVOLVE_EVAL_RUNTIME`` env override > the YAML
...
class CodingBenchEvalConfig:
    ...
    # Which registry those images are pulled from. Needed for the SWE-bench family on this
    # cluster: the tasks carry no [environment].docker_image, so the runner builds the name from
    # namespace/tag, and the default registry is Docker Hub, which the cluster cannot reach --
    # the pull returns no Docker-Content-Digest and every trial dies at image resolution. The
    # baseline configs all point at the in-cluster mirror; without this field a campaign cannot
    # say the same thing and evaluates nothing.
    benchmark_registry: "str | None" = None
    # Which beagle agent is the benchmarked evolvee. ``monet`` (the default) keeps the
    # fully-wired monet path below byte-identical; ANY other registered name (e.g.
    # ``mini-swe``) selects the GENERAL bridge in ``_agent_block`` — a lean ``{name, config}``
    # block the eval resolves through ``beagle.agents.build(name)``, so the evolvee is chosen by
    # the config's ``evolvee.harness.name``, not hardcoded here. Populated from the campaign
    # config's evolvee-block ``name`` (see ``from_self_evolve_config``); ``eval_agent_config``
    # carries that evolvee's own adapter knobs (provider / effort / config_path / forward_env / …).
    eval_agent_name: str = "monet"
    eval_agent_config: "dict | None" = None
    ...
    # self-evolve YAML ``runtime:`` block or the ``DARWINX_EVOLVE_EVAL_RUNTIME``
    ...
    def from_self_evolve_config(
        ...
        *,
        ...
        Runtime selection precedence (highest first):
          1. ``$DARWINX_EVOLVE_EVAL_RUNTIME`` env var (``local`` |
        ...
        if config_path is not None:
            ...
            if isinstance(monet, dict):
                # Evolvee-agnostic selector: the beagle registry NAME of the benchmarked agent +
                # that agent's own adapter config knobs. Absent (legacy monet campaigns, whose block
                # carries no ``name``) → the dataclass default ``monet`` and the fully-wired monet
                # path in ``_agent_block``. When present and non-monet, ``_agent_block`` emits the
                # general bridge instead. ``config`` is passed through verbatim to that bridge.
                name = monet.get("name")
                if isinstance(name, str) and name.strip():
                    kwargs["eval_agent_name"] = name.strip()
                extra = monet.get("config")
                if isinstance(extra, dict) and extra:
                    kwargs["eval_agent_config"] = dict(extra)
            ...
            if isinstance(bench, dict):
                ...
                rg = bench.get("registry")
                if isinstance(rg, str) and rg.strip():
                    kwargs["benchmark_registry"] = rg.strip()
...
# Config translation
# ──────────────────────────────────────────────────────────────────────
def _model_block(cb_cfg) -> dict:
    """The runner's model block. Agent-specific wiring is the adapter's job: a non-monet evolvee
    is resolved through beagle's registry (see ``_general_agent_block``) and brings its own
    provider/endpoint handling, so nothing here branches on which agent is being evaluated."""
    return {
...
def _general_agent_block(cb_cfg, forward_env):
    """The benchmarked-agent block for ANY non-monet beagle evolvee (e.g. mini-swe).

    A lean ``{name, config}``: the named beagle adapter (resolved via ``beagle.agents.build`` when
    the eval loads this config) owns its own install/run defaults, so the bridge only threads the
    code version (``agent_source`` = the evolvee experiment copy @ ref) + budgets + the evolvee's
    own config knobs (``eval_agent_config``: provider / effort / config_path / forward_env / …).
    No monet-specific ``install_cmd`` / ``monet_args`` — that omission is exactly what keeps this
    general. Explicit evolvee knobs win over the bridge defaults (``setdefault``)."""
    cfg = dict(cb_cfg.eval_agent_config or {})
    cfg.setdefault("forward_env", forward_env)
    cfg.setdefault("max_turns", cb_cfg.max_turns)
    cfg.setdefault("timeout", cb_cfg.timeout)
    token_env = cfg.get("token_env") or cb_cfg.monet_token_env
    cfg.setdefault("agent_source", {
        "type": "git_clone",
    ...
    return {"name": cb_cfg.eval_agent_name, "config": cfg}


def _agent_block(cb_cfg, forward_env):
    """Agent config for the runner. A non-monet evolvee emits the GENERAL bridge block
    (``_general_agent_block``) the eval resolves via ``beagle.agents.build``; otherwise the monet
    path below — BROWSER mode (webarena-infinity) emits the browser monet_args + npm-ci install;
    else the terminal monet path (both unchanged)."""
    if cb_cfg.eval_agent_name != "monet":
        return _general_agent_block(cb_cfg, forward_env)
    src = {
        "agent_source": {
            "type": "git_clone",
            "repo_url": cb_cfg.monet_repo_url,
    ...
    # Env overrides for the evolvee's budget, mirroring DARWINX_EVAL_EFFORT above. Without
    # these two the knobs reach the plain-eval path but not this one, so a host that sets
    # a budget sees it silently ignored for every evolution eval — the failure mode is a
    # campaign that looks configured and isn't. Non-positive or unparseable values are
    # ignored rather than raising: a bad budget should not abort a running campaign.
    src["max_turns"] = _positive_int_env("DARWINX_EVAL_MAX_TURNS", cb_cfg.max_turns)
    src["timeout"] = _positive_int_env("DARWINX_EVAL_TIMEOUT", cb_cfg.timeout)
    return {"name": "monet", "config": src}


def _positive_int_env(name: str, default: int) -> int:
    """``int`` from env when it parses to > 0, else ``default``."""
...
def _benchmark_block(cb_cfg, task_ids):
    ...
    if cb_cfg.benchmark_registry:
        b["registry"] = cb_cfg.benchmark_registry
...
#: An input-size refusal names itself; these are the phrasings seen from the gateway and from
#: litellm. Kept narrow on purpose: a 400 for any other reason is not the agent's doing.
...
def _row_is_context_overflow(row) -> bool:
    """The agent filled its own context until the model refused the request.

    Scored as a failure rather than dropped. Dropping removes the trial from the denominator, so
    an agent that overflows more often has more of its worst runs excluded and its measured rate
    rises on the remainder -- and since DarwinX edits its own prompts, "become more verbose" would
    be a way to score better without solving more tasks. Same reasoning the codebase already
    applies to agent timeouts: the agent ran and blew its budget, which is a measurement.
    """
    err = str(row.get("error") or "")
    return any(m in err for m in _CONTEXT_OVERFLOW_MARKERS)
...
def _rows_to_samples(rows, *, absorb_timeouts, absorb_transient):
    ...
    for row in rows:
        ...
        if reward is None:
            if row.get("error"):
                # An input-size refusal is the agent's own budgeting failure, so it counts as a
                # loss. Dropping it would take it out of the denominator, and an agent that
                # overflows more often would score better on what remains -- reachable here,
                # since the proposer edits the prompts that decide context size.
                if _row_is_context_overflow(row):
                    samples.setdefault(base, []).append(0.0)
                continue
...
def _docker_reachable_host_ip() -> str:
    override = os.environ.get("DARWINX_EVAL_DOCKER_HOST_PROXY_HOST")
    if override:
        return override
...
# ``DEFAULT_MONET_REPO_URL`` — the OWNED ``your-org`` fork), NOT
...
_CANDIDATE_INJECTION_ENV = "DARWINX_EVOLVE_CANDIDATE_COMMIT_INJECTION"
...
def _git(args: list[str], *, cwd: Path, timeout: int = 300) -> subprocess.CompletedProcess:
    ...
    A timeout is reported as a failed process rather than raised. Callers here
    already branch on ``returncode`` and degrade gracefully, but an escaping
    ``TimeoutExpired`` propagates all the way out and kills the pipeline: that
    is how a base-Monet bootstrap died tonight, taking the supervisor with it
    before it could spawn a single worker. Object enumeration against this
    repo runs on Lustre and blows multi-minute budgets routinely, so treat a
    slow git as a failed git.
    """
    try:
        ...
        return subprocess.CompletedProcess(
            ...
            stderr=f"git {' '.join(args[:2])} timed out after {timeout}s",
        )
...
def _create_ref_via_api(repo_url: str, branch: str, sha: str) -> bool:
    """Create ``refs/heads/<branch>`` -> ``sha`` through the GitHub API.

    ``git push`` has to enumerate objects in a clone that lives on Lustre,
    which regularly exceeds any sane timeout even when the commit is already
    published and the push would be a no-op ref update. The API does that ref
    update without touching the local object store, so it turns a multi-minute
    stall into one request.

    Only works when the remote already has the object (root/bootstrap evals,
    re-evals of published nodes); a genuinely new candidate commit still needs
    a real push, which the caller falls back to.
    """
    ...
    if not m:
        return False
    api = f"https://api.github.com/repos/{m.group(1)}/{m.group(2)}/git/refs"
    ...
    try:
        ...
        # 422 covers BOTH "Reference already exists" (a real success: the ref
        # points where we want) and "Object does not exist" (the remote has never
        # seen this commit, so NOTHING was created). Treating the latter as
        # success is catastrophic and silent: the caller skips the git-push
        # fallback, logs "API-created branch", and then every trial container
        # dies on `git clone --branch <branch>` with exit 128 "Remote branch not
        # found". The eval scores 0 / errors out, the node inherits its parent's
        # commit and score, and the phantom guard files it as `no_change` — so a
        # missing ref masquerades as "the proposer's change did nothing".
        # Observed 2026-07-28: it wasted 231+ trials across both clean-base
        # campaigns and made every evolved candidate unmeasurable, because a
        # freshly committed candidate is exactly the case where the object is not
        # yet on the remote.
        if e.code != 422:
            return False
        try:
            ...
            # Body unreadable: assume the dangerous case and let the caller push.
            return False
...
def _publish_candidate_commit(
    *, monet_dir: Path, job_name: str | None, repo_url: str,
    ...
    # Cheap path first: if the remote already has this object, the API creates
    # the ref without git enumerating the Lustre-backed object store at all.
    if _create_ref_via_api(repo_url, branch, sha):
        print(
            f"[codingbench_eval] candidate-commit injection: evaluating {sha[:12]} "
            f"via API-created branch {branch} on {repo_url}",
            file=sys.stderr,
        )
        return _CandidateRef(
            branch=branch, repo_url=repo_url, sha=sha, monet_dir=monet_dir,
        )
    push = _git(
        ["push", "--force", repo_url, f"{sha}:refs/heads/{branch}"],
        cwd=monet_dir, timeout=900,
    )
    if push.returncode != 0:
        # A slow push is not the same as a rejected one: the ref often lands
        # server-side while git is still enumerating objects over Lustre. Ask
        # the remote before falling back, because the fallback silently
        # evaluates the DEFAULT ref -- i.e. scores a different agent than the
        # candidate under test.
        ls = _git(
            ["ls-remote", "--heads", repo_url, f"refs/heads/{branch}"],
            cwd=monet_dir, timeout=180,
        )
        if ls.returncode == 0 and branch in (ls.stdout or ""):
            print(
                "[codingbench_eval] candidate-commit injection: push reported "
                f"failure but {branch} is present on the remote; continuing",
                file=sys.stderr,
            )
            return _CandidateRef(
                branch=branch, repo_url=repo_url, sha=sha, monet_dir=monet_dir,
...
def _maybe_inject_candidate_commit(
    *, cwd: Path | None, job_name: str | None, cb_cfg: CodingBenchEvalConfig,
    ...
    if not _candidate_injection_enabled() or cwd is None:
        return cb_cfg, None
    monet_dir = Path(cwd) / AGENT_SUBMODULE
...
#
# Attempts are read from ``DARWINX_EVOLVE_EVAL_ATTEMPTS`` (campaign-wide) and
...
_EVAL_ATTEMPTS_ENV = "DARWINX_EVOLVE_EVAL_ATTEMPTS"
...
# Benchmarks whose ``resolved`` in run.json is trajectory-completion until the
# official grader has run. Every one of these MUST be graded before its score is
# read; see the docstring below for what happens otherwise.
_POSTHOC_GRADED = frozenset({
    "swe-bench-verified", "swe-bench-lite", "swe-bench-pro",
})


def _grade_swebench_and_rewrite(run_dir, cb_cfg) -> None:
    """SWE-bench is 2-phase: ``runner.run`` only writes ``predictions.json``
    ...
    the REAL graded verdict, so the self_evolve loop optimizes test-pass.
    (Assumes pass@1: one patch per instance in predictions.json.)

    Applies to every post-hoc-graded benchmark, not just Verified. This was scoped
    to ``swe-bench-verified`` alone until 2026-08-09, which made it a silent no-op
    on Pro and Lite — and the ungraded number is not merely noisy, it is close to
    constant. On an 18-task Pro smoke, Monet's ungraded ``resolved`` read 17/18
    (94%) against a real graded 5/18 (27.8%): seven of those "passes" had
    ``patch_len=0`` and ``applied=False``, i.e. the agent changed nothing. What
    that field actually tracks is "the agent exited 0", so a Pro campaign reading
    it would have selected for processes that terminate cleanly, which every
    harness already does. A benchmark added here without a grader registered in
    ``runner.run.BENCHMARK_GRADERS`` would be worse than an unused benchmark.
    """
    import re as _re
    bench = (getattr(cb_cfg, "benchmark_name", "") or "").strip()
    if bench not in _POSTHOC_GRADED:
        return
    run_dir = Path(run_dir)
    preds = run_dir / "predictions.json"
    if not preds.exists():
        return
    runtime_kind = (os.environ.get("DARWINX_EVOLVE_EVAL_RUNTIME") or "xrlenv-cluster").strip()
    try:
        nw = int(os.environ.get("DARWINX_EVAL_HARBOR_N_CONCURRENT", "6"))
    except ValueError:
        nw = 6
    if bench == "swe-bench-pro":
        # Pro's grader takes no ``dataset``: the instance set is pinned by the
        # vendored sweap sample rather than selected per run.
        from benchmarks.swe_bench_pro import grading as _pro_grading
        overall = _pro_grading.grade(
    ...
    # Recompute the roll-up. It is written pre-grading from trajectory
    # completion and was never revisited, so anything reaching for ``totals``
    # instead of ``per_task_results`` read the ungraded count -- 17 rather than
    # 5 on the Pro smoke. The self_evolve loop reads the per-task rows and is
    # unaffected, but this field has talked two separate analyses of
    # ``mini_dev_0801`` into diagnosing bugs that did not exist.
    totals = d.get("totals")
    if isinstance(totals, dict):
        graded = [
            row for row in (d.get("per_task_results") or [])
            if _re.sub(r"__s\d+$", "", str(row.get("task_id") or "")) in resolved
        ]
        totals["num_tasks_resolved"] = sum(1 for row in graded if row.get("resolved"))
        totals["num_tasks_graded"] = len(graded)
...
def _run(
    *,
    ...
    # any campaign-wide ``DARWINX_EVAL_HARBOR_N_CONCURRENT`` setting). Read
...
#
# Set ``DARWINX_EVOLVE_TOLERATE_TASK_INFRA_FAILURE=0`` to restore the strict
# "abort on any infra failure" behaviour.
_TOLERATE_TASK_INFRA_ENV = "DARWINX_EVOLVE_TOLERATE_TASK_INFRA_FAILURE"
...
# ``DARWINX_GATE_ABSORB_TRANSIENT_INFRA=0``.
...
# When ``DARWINX_EVAL_ABSORB_TIMEOUTS=1`` we drop timeout trials from the score
...
def _absorb_contention_timeouts() -> str:
    """Absorption mode: ``"off"``, ``"row"`` or ``"job"``.

    ``row`` (``1``/``true``/``on``) is the historical behaviour: drop every
    agent-timeout trial from the denominator, and abort the job if that leaves
    nothing. ``job`` keeps only the second half — a wholly timed-out job is
    still an infra failure, but a *partial* timeout counts as the failure it is.

    ``job`` is the right setting for an evolution campaign. Dropping timeouts
    row-wise is unbiased only if the timeout rate is a property of the cluster;
    it is not, it is partly a property of the harness, and a child that makes
    the agent loop or thrash has exactly those trials deleted from its
    denominator. That is a scoring gradient pointing at timeout-inducing
    harnesses, and unlike contention noise it does not average out over a
    campaign — selection compounds it. Contention that hits every arm of a
    fixed shared panel is noise the null model already prices in; absorption
    converts it into bias, which the null model cannot.
    """
    val = os.environ.get("DARWINX_EVAL_ABSORB_TIMEOUTS", "0").strip().lower()
    if val in {"job", "whole_job", "wholejob"}:
        return "job"
    return "row" if val in {"1", "true", "yes", "on"} else "off"
...
# baseline by exporting ``DARWINX_EVAL_HARBOR_N_CONCURRENT`` into the eval
...
_EVAL_PARALLELISM_ENV = "DARWINX_EVAL_HARBOR_N_CONCURRENT"
...
def _build_eval_result(
    ...
    *,
    ...
    # error) when DARWINX_EVAL_ABSORB_TIMEOUTS=1 — same rationale as the transient
    # absorption above, but for cluster-contention timeouts. Default off.
    excluded_timeout: list[str] = []
    _timeout_mode = _absorb_contention_timeouts()
    if _timeout_mode != "off":
        kept_rows = []
        timed_out: list[str] = []
        for r in rows:
            if _row_is_agent_timeout(r):
                timed_out.append(str(r.get("task_id")))
            else:
                kept_rows.append(r)
        # A job with nothing but timeouts says more about the cluster than the
        # harness, in either mode: 0.0 would be a fabricated measurement.
        if timed_out and not kept_rows:
            failures = _infrastructure_failures_from_raw(raw, run_dir)
            raise EvalInfrastructureError(
        ...
        if timed_out and _timeout_mode == "row":
            excluded_timeout = timed_out
            print(
            ...
            print(
                "[codingbench_eval] agent-timeout: scoring "
                f"{len(timed_out)} timed-out trial(s) in {run_dir.name} as "
                f"failures (e.g. {timed_out[:3]}); absorb mode=job keeps only "
                "the whole-job guard, so a harness cannot shed the trials it "
                "made slow",
                file=sys.stderr,
            )
