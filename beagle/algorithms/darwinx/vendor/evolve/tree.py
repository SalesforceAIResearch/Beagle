...
def root_search_eval(conn: sqlite3.Connection, *, campaign: str) -> NodeEval | None:
    """The root's eval, whichever kind it was recorded under.

    ``root_full_eval`` matches ``eval_kind='root_full'`` only, and that kind is written exclusively by
    the adaptive-subset path. A campaign with ``adaptive_subset`` off records the root's baseline as
    ``subset_final``, so callers asking for the root's full-width measurement get None and silently
    fall back to whatever the current node knows. This accepts either kind, preferring ``root_full``
    when both exist (it is the wider measurement), then oldest first.
    """
    row = conn.execute(
        """
        SELECT e.* FROM node_evals e
        JOIN nodes n ON n.id = e.node_id
         WHERE e.campaign = ? AND n.parent_id IS NULL
           AND e.eval_kind IN ('root_full', 'subset_final')
         ORDER BY CASE e.eval_kind WHEN 'root_full' THEN 0 ELSE 1 END,
...
#
# Override with the env var DARWINX_EVAL_REPORTS_ROOT or the --reports-root flag
# on every entry script.
def _default_reports_root() -> Path:
    env = os.environ.get("DARWINX_EVAL_REPORTS_ROOT")
    if env:
