...
def probe_init_model(
    ...
    *,
    ...
    workspace.mkdir(parents=True, exist_ok=True)
    # --trust is required, not merely convenient: cursor-agent gates an
    # untrusted workspace behind an interactive "Workspace Trust Required"
    # prompt and never emits the system init event, so the probe below times
    # out and the supervisor aborts the whole campaign before it writes a
    # state.db. Campaigns run in freshly created worktrees, and the trust
    # store does not survive a box reset, so untrusted is the normal case.
    cmd = [
        cursor_bin, "-p", "--force", "--trust",
        "--output-format", "stream-json",
        "--workspace", str(workspace),
