...
Review the diff in `{{ wt_dir }}/agents/mini_swe_agent/vendor/`, run focused checks, fix bugs you
...
Run full `python -m pytest` only when the change is broad or focused evidence suggests wider risk.
...
Steps:
1. Inspect `git -C {{ wt_dir }}/agents/mini_swe_agent/vendor diff {{ parent_commit }}..HEAD`.
