...
def _prompt_path(name: str) -> Path:
    ...
    the defaults. Keeps one prompt set authoritative instead of forking all 16.

    It accepts a ``os.pathsep``-separated *list*, searched left to right, so a
    variant can layer on another variant instead of copying it. That matters
    because the surfaces differ per benchmark: mini's prompt surface is
    ``config/benchmarks/swebench.yaml`` on SWE-V but ``config/mini.yaml`` on
    TB2.1 (Harbor runs the console script, which reads the latter). Without
    layering, a TB2.1 mini prompt set would have to duplicate every mini file to
    correct two of them, and the copies would drift.
    """
...
def _final_eval_archive_max_bytes() -> int:
    raw = os.environ.get("DARWINX_EVAL_FINAL_ARCHIVE_MAX_BYTES")
    if raw is None:
...
def _infra_eval_retries() -> int:
    ...
    DARWINX_EVAL_INFRA_RETRIES=0 to restore the old fail-on-first-infra-error
    ...
    raw = os.environ.get("DARWINX_EVAL_INFRA_RETRIES", "6").strip()
    try:
        return max(0, int(raw))
    except ValueError:
        return 6
...
def _eval_heartbeat_interval_s() -> float:
    raw = os.environ.get("DARWINX_EVAL_EVAL_HEARTBEAT_INTERVAL_S")
    if raw is None:
...
def _progress_signal_on() -> bool:
    """MOSS-style graded progress signal (DARWINX_GATE_PROGRESS_SIGNAL=1).
    ...
    """
    return os.environ.get("DARWINX_GATE_PROGRESS_SIGNAL", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _additive_scope_on() -> bool:
    """Additive-scope constraint (DARWINX_GATE_ADDITIVE_SCOPE=1).
    ...
    """
    return os.environ.get("DARWINX_GATE_ADDITIVE_SCOPE", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def _additive_max_deletions() -> int:
    ...
    try:
        return max(0, int(os.environ.get("DARWINX_GATE_MAX_DELETIONS", "40")))
    except ValueError:
        return 40


def _prune_enabled() -> bool:
    """Subtraction as a first-class variant (DARWINX_GATE_PRUNE_ENABLED=1).

    Every other variant kind only ADDS: the proposer prompt forbids deletion, the
    additive-scope guard bounds it, and the preserve-extend contract names
    earlier-added skills as invariants. An evolved agent therefore accumulates
    modifications monotonically -- across all prior campaigns not one skill file
    was ever deleted -- so dead cues, redundant procedures, and locally-helpful
    but globally-harmful guards pile up with no way out.

    When on, a fraction of nodes (``DARWINX_GATE_PRUNE_RATE``) are PRUNE nodes: they
    get the subtraction prompt, are exempt from the deletion guards, and can be
    accepted for preserving behavior while shrinking the agent. Default off, so a
    control arm is byte-identical to the additive-only pipeline.
    """
    return os.environ.get("DARWINX_GATE_PRUNE_ENABLED", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _prune_rate() -> float:
    """Fraction of eligible nodes drawn as PRUNE nodes (default 0.2).

    Kept well below 0.5: subtraction is a maintenance operation on what the
    additive nodes produced, so it only has material to work on if additive
    nodes remain the majority.
    """
    try:
        return min(1.0, max(0.0, float(os.environ.get("DARWINX_GATE_PRUNE_RATE", "0.2"))))
    except ValueError:
        return 0.2


def _prune_min_lineage() -> int:
    """Accumulated lineage commits required before a node may be a PRUNE (default 3).

    A prune node needs something to remove. Near the root -- which for a clean
    base-rooted campaign is a pristine agent -- there are no accumulated
    modifications, so drawing a prune there wastes the node.
    """
    try:
        return max(1, int(os.environ.get("DARWINX_GATE_PRUNE_MIN_LINEAGE", "3")))
    except ValueError:
        return 3


def _guard_autoexclude_on() -> bool:
    """Automatically retire guard tasks that veto many unrelated candidates
    (DARWINX_GATE_GUARD_AUTOEXCLUDE=1). Default off: silently dropping a guard is a
    real loss of regression coverage, so it must be asked for."""
    return os.environ.get("DARWINX_GATE_GUARD_AUTOEXCLUDE", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _guard_autoexclude_threshold() -> int:
    """Distinct pipelines a task must veto before it is retired (default 4).

    Four unrelated diffs failing the same task is weak evidence against all four
    and strong evidence against the task. Set high enough that a genuinely
    fragile area of the agent is not de-guarded after one bad run.
    """
    try:
        return max(2, int(os.environ.get("DARWINX_GATE_GUARD_AUTOEXCLUDE_THRESHOLD", "4")))
    except ValueError:
        return 4


def _consolidate_enabled() -> bool:
    """Rewrite-for-simplicity as a first-class variant (DARWINX_GATE_CONSOLIDATE_ENABLED=1).

    PRUNE can only DELETE, and only what this campaign's lineage added -- its
    prompt names stock product functionality as off-limits and its verdict rule
    demands a deletion-dominated diff. That covers dead weight but not the move
    that actually makes a harness better: rewriting several accumulated special
    cases into one general mechanism, or replacing a pre-evolve component the
    agent has outgrown. Such a rewrite is often line-neutral, so every existing
    variant rejects it -- additive nodes because it deletes, prune nodes because
    it adds.

    A CONSOLIDATE node is exempt from the additive and extension contracts, may
    edit base/pre-evolve code, and is accepted for holding capability while
    measurably lowering harness complexity. Default off so an additive-only
    control arm stays byte-identical.
    """
    return os.environ.get("DARWINX_GATE_CONSOLIDATE_ENABLED", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _consolidate_rate(depth: int) -> float:
    """Fraction of eligible nodes drawn as CONSOLIDATE, rising with lineage depth.

    The target trajectory is grow-THEN-simplify: early on there is little
    accumulated structure worth folding together, and spending nodes on rewriting
    a pristine base buys nothing. Later, the accumulated special cases are exactly
    what is holding the score down. A flat rate would interleave the two phases
    and blur the trend we are trying to demonstrate, so the rate ramps instead.
    """
    try:
        base = min(1.0, max(0.0, float(os.environ.get("DARWINX_GATE_CONSOLIDATE_RATE", "0.3"))))
    except ValueError:
        base = 0.3
    try:
        late = min(1.0, max(0.0, float(os.environ.get("DARWINX_GATE_CONSOLIDATE_RATE_LATE", "0.5"))))
    except ValueError:
        late = 0.5
    try:
        late_depth = max(1, int(os.environ.get("DARWINX_GATE_CONSOLIDATE_LATE_DEPTH", "5")))
    except ValueError:
        late_depth = 5
    return late if depth >= late_depth else base


def _consolidate_min_lineage() -> int:
    """Lineage depth before CONSOLIDATE may be drawn (default 2).

    Lower than PRUNE's 3: a consolidation does not need accumulated material to
    delete, because it may also rewrite pre-evolve code. But at depth 0-1 the
    agent is still essentially the stock harness and the growth phase has barely
    started, so drawing one there is premature.
    """
    try:
        return max(0, int(os.environ.get("DARWINX_GATE_CONSOLIDATE_MIN_LINEAGE", "2")))
    except ValueError:
        return 2
...
def _hybrid_archive_on() -> bool:
    ...
    """
    return os.environ.get("DARWINX_GATE_HYBRID_ARCHIVE", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _defer_node_full_eval() -> bool:
    """Defer the per-node FULL eval (DARWINX_GATE_DEFER_NODE_FULL_EVAL=1).
    ...
    """
    return os.environ.get("DARWINX_GATE_DEFER_NODE_FULL_EVAL", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _fixed_eval_panel() -> bool:
    """Score every node of a campaign on ONE shared panel (DARWINX_GATE_FIXED_EVAL_PANEL=1).

    Without this, ``_defer_node_full_eval`` scores each node on its own
    ``claimed + guards`` set, so the denominator is chosen by the node. Since the
    numerator (tasks the harness can actually solve) barely moves between nodes,
    claiming fewer tasks raises the score arithmetically, and parent selection
    ranks on exactly that score. Campaign ``mini_dev_0801`` is the worked example:
    every one of its 11 scored nodes solved the same two tasks, and the only score
    change in 14 hours was the panel shrinking 12 -> 6 (0.167 -> 0.333), which the
    search read as an improvement and inherited down the whole lineage.

    A shared panel keeps the cheap-minibatch cost model but makes scores
    comparable, which is the precondition for a learning curve meaning anything.
    """
    return os.environ.get("DARWINX_GATE_FIXED_EVAL_PANEL", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }


def _eval_panel_size() -> int:
    """Size of the shared panel. 0 (default) = use the whole campaign subset."""
    try:
        return max(0, int(os.environ.get("DARWINX_GATE_EVAL_PANEL_SIZE", "0")))
    except ValueError:
        return 0


def _panel_digest(tasks: list[str]) -> str:
    """Stable id for a panel, so mixed-panel campaigns are detectable after the
    fact instead of silently comparing 2/6 against 2/12."""
    import hashlib
    joined = "\n".join(sorted(tasks))
    return hashlib.sha256(joined.encode()).hexdigest()[:12]


def _build_fixed_panel(subset_tasks: list[str], campaign: str) -> list[str]:
    """Deterministic shared panel: the campaign subset, optionally subsampled to
    ``DARWINX_GATE_EVAL_PANEL_SIZE``. Seeded by campaign name so a resumed campaign
    rebuilds the identical panel rather than silently switching denominators."""
    tasks = list(dict.fromkeys(subset_tasks))
    size = _eval_panel_size()
    if size <= 0 or size >= len(tasks):
        return tasks
    import random as _random
    rng = _random.Random(f"{campaign}:panel:{size}")
    return sorted(rng.sample(tasks, size))
...
def _archive_max_regressions() -> int:
    ...
    try:
        return max(0, int(os.environ.get("DARWINX_GATE_ARCHIVE_MAX_REGRESSIONS", "25")))
    except ValueError:
        return 25


def _bestof2_contrast_on() -> bool:
    """Best-of-2 contrastive self-analysis (DARWINX_GATE_BESTOF2_CONTRAST=1).
    ...
    """
    return os.environ.get("DARWINX_GATE_BESTOF2_CONTRAST", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def _fractional_gate_on() -> bool:
    ...
    majority vote (DARWINX_GATE_FRACTIONAL_GATE=1). The binary ``rate>=0.5 -> 1.0``
    ...
    return _env_on("DARWINX_GATE_FRACTIONAL_GATE")
...
def _regression_margin() -> float:
    ...
    jitter. Default 0.20."""
    return _env_float("DARWINX_GATE_REGRESSION_MARGIN", 0.20)
...
def _knowledge_gate_on() -> bool:
    ...
    collective-knowledge activation function (DARWINX_GATE_KNOWLEDGE_GATE=1)."""
    return _env_on("DARWINX_GATE_KNOWLEDGE_GATE")
...
def _failure_theme_digest() -> str:
    """Campaign-wide FAILURE-MODE THEME digest (DARWINX_GATE_FAILURE_THEME=1).
    ...
    if not _env_on("DARWINX_GATE_FAILURE_THEME"):
        return ""
    ...
    try:
        ...
        if run_json and _os.path.exists(run_json):
            from dx_trace import failure_mode as _fm
...
def _archive_all_on() -> bool:
    ...
    (DARWINX_GATE_ARCHIVE_ALL=1). Realizes 'never directly REJECT, archive as needed'.
    Promotion of the shipped best stays strict (clean avg@k win only)."""
    return _env_on("DARWINX_GATE_ARCHIVE_ALL")


def _qd_archive_on() -> bool:
    """Quality-diversity archive (DARWINX_GATE_QD_ARCHIVE=1).
    ...
    return _env_on("DARWINX_GATE_QD_ARCHIVE")


def _qd_solved_threshold() -> float:
    """avg@k pass-rate STRICTLY ABOVE which a claimed task counts as cracked for
    the QD *archive* gate (DARWINX_GATE_QD_SOLVED_THRESHOLD, default 0.0 => any seed).
    ...
    return _env_float("DARWINX_GATE_QD_SOLVED_THRESHOLD", 0.0)


def _guard_count(default: int = 3) -> int:
    """Number of regression-guard tasks per node (DARWINX_EVAL_GUARD_COUNT).
    Larger = better regression coverage of the ~84 non-claimed tasks. The legacy
    default was 3 (~3.5% coverage)."""
    return max(0, _env_int("DARWINX_EVAL_GUARD_COUNT", default))


# ─── Configuration ───────────────────────────────────────────────────────


def _stage_timeout(stage: str, default: int) -> int:
    """Per-stage proposer wall-clock cap, overridable by env.

    DARWINX_GATE_<STAGE>_TIMEOUT_S, e.g. DARWINX_GATE_ANALYZE_TIMEOUT_S=3600. Exists because
    a quarter of mini_smoke_0801b's iterations were lost to the analyze stage
    hitting the old hardcoded 1800s, and diagnosing that needs the cap to be a
    dial rather than an edit. Ignores unparseable or non-positive values so a
    typo cannot silently disable the cap entirely.
    """
    raw = os.environ.get(f"DARWINX_GATE_{stage}_TIMEOUT_S", "").strip()
    if raw:
        try:
            val = int(raw)
            if val > 0:
...
class PipelineConfig:
    ...
    # 2/5 fluke no longer counts as a win. Override via DARWINX_EVAL_MINI_EVAL_K.
    ...
    # byte-identical; the v2 launcher sets DARWINX_EVAL_FULLSET_METRIC=avg.
    ...
    def analyze_timeout_s(self) -> int:
        return _stage_timeout("ANALYZE", self.cursor_analyze_timeout_s or self.cursor_timeout_s)

    @property
    def implement_timeout_s(self) -> int:
        return _stage_timeout("IMPLEMENT", self.cursor_implement_timeout_s or self.cursor_timeout_s)

    @property
    def review_timeout_s(self) -> int:
        return _stage_timeout("REVIEW", self.cursor_review_timeout_s or self.cursor_timeout_s)

    @property
    def picker_timeout_s(self) -> int:
        return _stage_timeout("PICKER", self.cursor_picker_timeout_s or self.cursor_timeout_s)
...
class SelfEvolvePipeline:
    ...
    def __init__(self, cfg: PipelineConfig) -> None:
        self.cfg = cfg
        # Rebuild the injected evolver, if the campaign names one. This has to happen
        # per process, not once at launch: the supervisor spawns each pipeline as a
        # separate worker, and a Python object injected in the parent does not survive
        # the process boundary. Every worker therefore constructs this class, which
        # makes it the one reliable place to re-establish the seam. No-op (returns None)
        # for campaigns that don't name an evolver, so the native proposer is unaffected.
        run_config.apply_driver_knobs(cfg.config_path)
        run_config.activate_evolver_from_config(cfg.config_path)
        ...
        # PRUNE lottery result, resolved once on first use. Prompt selection, the
        # deletion-guard bypass, and the acceptance carve-out all read it, and a
        # re-draw between those points would produce an incoherent node (pruning
        # prompt, additive gate), so it must be decided exactly once.
        self._prune_node: bool | None = None
    ...
    def _preservation_solved_pool(self, node: tree.Node) -> list[str]:
        ...
        The adaptive-subset harness only re-measures a fixed ~10% canary
        ...
        Disable via ``DARWINX_GATE_FULL_SOLVED_PRESERVATION=0`` to restore the
        legacy parent-only (canary-shadow) pool.
        """
        pool: list[str] = [t for t in (self._node_solved_tasks(node) or []) if t]
        seen = set(pool)
        if os.environ.get(
            "DARWINX_GATE_FULL_SOLVED_PRESERVATION", "1"
    ...
    def _persist_node_eval_result(
        ...
        *,
        ...
        # Stamp every eval row with the identity of the panel it was scored on.
        # A curve is only readable if its points share a denominator, and the
        # way that fails is silent: mini_dev_0801's one apparent improvement was
        # 2/12 becoming 2/6. Recorded here rather than at the call sites because
        # the baseline anchors the curve, and it was the single row that had no
        # digest -- so the comparison that matters most, first child against
        # baseline, was the one that could not be checked.
        panel = list(result.task_names)
        metadata = dict(metadata or {})
        metadata.setdefault("panel_digest", _panel_digest(panel))
        metadata.setdefault("panel_size", len(panel))
    ...
    def _unscored_orphan_root_id(self) -> str | None:
        """This campaign's root, if it exists and nothing in the tree is scored.

        The recovery path for a campaign whose first attempt bootstrapped a root
        and then died — a crash between bootstrap and Phase 2 leaves a tree that
        every later attempt refuses to start from. See `_select_parent`.

        Deliberately narrow: it returns None as soon as any node carries a score,
        so it can only ever rescue a campaign that never got off the ground, and
        cannot quietly reset a running campaign to its root.
        """
        scored = self.conn.execute(
            "SELECT 1 FROM nodes WHERE campaign = ? AND subset = ? "
            "AND score IS NOT NULL LIMIT 1",
            (self.cfg.campaign, self.cfg.subset_label),
        ).fetchone()
        if scored:
            return None
        return self._existing_root_id()
    ...
    def _select_parent(self) -> None:
        ...
        try:
            ...
            if not root_id:
                # An orphaned root: bootstrapped by an earlier attempt that died
                # before Phase 2 could score it. Without this the campaign is
                # wedged permanently — bootstrap is idempotent, so every retry
                # sees "root already exists", never sets _bootstrapped_root_id,
                # and raises here again. Recovery needed a hand-edited database,
                # which is not a thing an unattended multi-day run can wait for.
                #
                # Adopting it is safe precisely because nothing is scored yet:
                # there is no other node the strategy could have preferred, and
                # Phase 2 does to it exactly what it would have done on the
                # first attempt. The condition stops applying the moment any
                # node has a score, so a genuine "no eligible parent" later in
                # a campaign — which means something else is wrong — still
                # raises.
                root_id = self._unscored_orphan_root_id()
                if root_id:
                    self.log.warning(
                        "adopting orphaned root %s: it was bootstrapped by an "
                        "earlier attempt that died before scoring it",
                        root_id,
                    )
            if not root_id:
                raise
        ...
        if self.cfg.baseline_logs:
            ...
            if self.cfg.bootstrap_only:
                ...
                if bootstrap_nc is not None:
                    ...
                    extra_env = {"DARWINX_EVAL_HARBOR_N_CONCURRENT": str(bootstrap_nc)}
    ...
    def _populate_passing_pool(self, result: eval_runner.EvalResult | None = None) -> None:
        ...
        unprobed. Gated by DARWINX_GATE_FULL_SOLVED_PRESERVATION. (legitimacy audit 2026-07-02)"""
        base: list[str] = []
    ...
    def _run_full(self, **kwargs):
        ...
        ``fullset_eval_metric == "avg"`` (env DARWINX_EVAL_FULLSET_METRIC) AND the
        ...
        eval ``DARWINX_EVAL_INFRA_RETRIES`` times (default 2, exponential-ish
        ...
        def _call_once():
            if metric == "avg" and not task_names:
                raise ValueError(
                    "DARWINX_EVAL_FULLSET_METRIC=avg requires an explicit task "
                    "list (DARWINX_EVAL_FULLSET_TASKS); refusing to silently fall "
    ...
    def _run_mini_eval(self, mini_tasks: list[str], *, iteration: int, _k_override: int | None = None):
        ...
        """
        env_k = os.environ.get("DARWINX_EVAL_MINI_EVAL_K", "").strip()
    ...
    def _run_mini_eval_screened(self, mini_tasks: list[str], *, iteration: int):
        ...
        verdict is BORDERLINE (coin-flip band 0.34-0.67) and DARWINX_GATE_ESCALATE_K>k
        ...
        try:
            esc_k = int(os.environ.get("DARWINX_GATE_ESCALATE_K", "0") or "0")
        except ValueError:
            esc_k = 0
    ...
    def _reasoned_verdict_decision(self, outcome: IterationOutcome) -> bool | None:
        ...
        the binary ``canary_passed`` screen. Env-gated (DARWINX_GATE_REASONED_VERDICT);
        ...
        try:
            ...
            _envk = os.environ.get("DARWINX_EVAL_MINI_EVAL_K", "").strip()
            ...
            # The measured panel null. Without it the GATE reads a +1/45 delta
            # on a panel whose null bar is 30/45 and cannot tell the two apart,
            # which is how a node gets promoted on noise. `load_from_env`
            # returns None unless DARWINX_GATE_PANEL_NULL points at a null file, so
            # campaigns that have not been given one are unaffected.
            from . import null_calibration as _nullcal
            evidence = rv.render_evidence(
                ...
                prune=self._is_prune_node(),
                consolidate=self._is_consolidate_node(),
                complexity=self._complexity_evidence() or None,
                panel_null=_nullcal.load_from_env(),
            )
            verdict = rv.decide(evidence, surface=surface)
            if verdict is None:
                return None
            _routed = os.environ.get("DARWINX_GATE_ROUTED_CODE", "0").strip() == "1"
            ...
            # Cross-benchmark generality gate (DARWINX_GATE_CROSS_BENCH_GATE): a
            # candidate that passed the in-domain screen is re-scored on a
            # HELD-OUT benchmark it was not evolved against. Regressing there
            # DOWNGRADES it (PROMOTE -> ARCHIVE) rather than destroying it —
            # §1.6 gate-as-enabler: explore liberally in the gate, confirm
            # rigorously downstream. A hard reject here would drop the node from
            # the hybrid archive, and archived specialists are exactly what
            # merge_selection.eligible_candidates recombines; the frozen-tip
            # post-mortem traced 0 merge candidates to precisely that.
            # No-op unless enabled AND configured; never blocks the loop.
            if passed:
                cb_res = self._cross_bench_result()
                if cb_res is not None:
                    from . import cross_bench as cbx
                    new_decision, reason = cbx.apply_cross_bench(
                        verdict.decision, cb_res
                    )
                    self.log.info("[cross-bench] %s", reason)
                    if new_decision.upper() != verdict.decision:
                        from dataclasses import replace as _dc_replace
                        self.log.info("cross-bench: %s->%s",
                                      verdict.decision, new_decision.upper())
                        verdict = _dc_replace(verdict, decision=new_decision.upper())
                        # the downgrade is itself a lesson for the next proposal
                        self._record_verdict_lesson(verdict)
                        passed = verdict.accept or verdict.archive
            # Multi-benchmark regression floor (DARWINX_GATE_MIXTURE_GATE). Where the
            # cross-bench gate holds ONE benchmark out, this one evolves against
            # several and guards each of them: no amount of aggregate gain buys a
            # regression on any single member. Same downgrade-not-destroy shape as
            # above, and it composes with it — both can only lower the decision, so
            # running both is never worse than running either.
            if passed:
                mx = self._mixture_floor_result()
                if mx is not None:
                    from . import multibench as mbx
                    new_decision, reason = mbx.apply_regression_floor(
                        verdict.decision, mx
                    )
                    self.log.info("[mixture] %s", reason)
                    if new_decision.upper() != verdict.decision:
                        from dataclasses import replace as _dc_replace
                        self.log.info("mixture: %s->%s",
                                      verdict.decision, new_decision.upper())
                        verdict = _dc_replace(verdict, decision=new_decision.upper())
                        self._record_verdict_lesson(verdict)
                        passed = verdict.accept or verdict.archive
    ...
    def _cross_bench_result(self):
        """Held-out cross-benchmark generality signal.

        Returns ``None`` when disabled / unconfigured / no signal (caller leaves
        the in-domain decision untouched), else the ``CrossBenchVerdict``. The
        caller combines it via ``cross_bench.apply_cross_bench``, which can only
        DOWNGRADE (PROMOTE -> ARCHIVE, keeping the node as a stepping stone) and
        never turns a REJECT into a PROMOTE. Returning the verdict rather than a
        veto boolean is deliberate: a boolean can only destroy the candidate,
        which is the frozen-tip failure §1.6 was written to prevent.

        The parent/root held-out baseline is supplied out-of-band
        (``DARWINX_GATE_HELDOUT_BASELINE``) so no parent re-checkout is needed.
        Fully defensive: any failure => ``None`` (never blocks the loop).
        """
        try:
            from . import cross_bench as cbx
            ...
            return cbx.cross_bench_verdict(
                baseline,
                child_rates,
                margin=cbx.regression_margin(),
                min_drop=cbx.min_mean_drop(),
            )
        except Exception as exc:  # noqa: BLE001 — never block the loop
            self.log.warning("cross-bench gate skipped (%s)", exc)
            return None

    def _mixture_floor_result(self):
        """Per-benchmark regression floor across the evolved mixture.

        Returns ``None`` when disabled / unconfigured / unmeasurable, so the in-domain
        decision stands untouched. Never raises: a gate that can crash the loop is worse
        than no gate.

        The comparison baseline comes from the spec (``DARWINX_GATE_MIXTURE_SPEC``), which is
        written by the baseline calibration campaign rather than by the evolution. That
        matters more here than in the single-benchmark case: with several benchmarks, a
        drifting yardstick lets a candidate trade one benchmark for another and still look
        like it improved on both.
        """
        try:
            from . import multibench as mbx
            if not mbx.gate_enabled():
                return None
            spec = mbx.load_mixture_spec()
            if not spec.names:
                self.log.warning("[mixture] gate enabled but spec is empty; skipping")
                return None
            base_cfg = eval_runner.CodingBenchEvalConfig.from_self_evolve_config(
                self.cfg.config_path
            )
            def measure(stage, only=None):
                return mbx.run_mixture_stage(
                    base_cfg,
                    spec,
                    stage=stage,
                    only=only,
            ...
            parent_rates = {n: spec.components[n].baseline for n in spec.names}
            tol_sd, min_abs = mbx.tolerance_sd(), mbx.min_abs_drop()

            # Stage 1: the screen. Small sample, wide tolerance -- a healthy candidate clears it
            # and the gate costs a fraction of a full pass. Three benchmarks at the full sample
            # would be hours per promotion, mostly spent confirming that nothing is wrong.
            got = measure("screen")
            if not got.rates:
                return None
            screen = mbx.regression_floor_verdict(
                parent_rates, got.rates, spec,
                tol_sd=tol_sd, min_abs=min_abs, n_used=got.n_used,
            )
            self.log.info("[mixture] screen: %s", screen.detail)
            if screen.passed:
                fit = mbx.mixture_fitness(got.rates, spec, gain_cap=mbx.gain_cap_sd())
                self.log.info("[mixture] %s", fit.detail)
                return screen

            # Stage 2: only the benchmarks that looked bad, at the full sample. A screen-sized
            # drop is weak evidence -- at 25 tasks its tolerance is ~11 points wide, so what
            # crosses it is often still noise. Vetoing on the screen alone would reject good
            # candidates; confirming costs one pass on one benchmark. The screen sample is a
            # prefix of the full one, so this adds tasks rather than repeating them.
            self.log.info("[mixture] escalating to the full sample on: %s",
                          ", ".join(screen.regressed))
            confirm = measure("full", only=list(screen.regressed))
            rates = {**got.rates, **confirm.rates}
            n_used = {**got.n_used, **confirm.n_used}
            final = mbx.regression_floor_verdict(
                parent_rates, rates, spec, tol_sd=tol_sd, min_abs=min_abs, n_used=n_used,
            )
            self.log.info("[mixture] confirm: %s", final.detail)
            fit = mbx.mixture_fitness(rates, spec, gain_cap=mbx.gain_cap_sd())
            self.log.info("[mixture] %s", fit.detail)
            return final
        except Exception as exc:  # noqa: BLE001 — never block the loop
            self.log.warning("mixture gate skipped (%s)", exc)
            return None
    ...
    def _diff_line_counts(self) -> tuple[int, int]:
        """(removed, added) real line counts of the candidate vs its parent commit.

        Used to verify a PRUNE node actually shrank the agent. File headers
        (``---``/``+++``) are excluded so a pure file deletion is not miscounted.
        Returns (0, 0) if the diff cannot be read, which makes a prune fall through
        to the phantom guard rather than be accepted on missing evidence.
        """
        removed = added = 0
        for line in (self._child_diff_text() or "").splitlines():
    ...
    def _record_verdict_lesson(self, verdict) -> None:
        ...
        try:
            ...
            rec = {
                ...
                # Marks subtraction nodes so a campaign's add-vs-remove balance is
                # recoverable from this file alone (scripts/surface_attribution.py
                # reads it), which is what the ablation needs to report.
                "prune": self._is_prune_node(),
                "consolidate": self._is_consolidate_node(),
    ...
    def _mixture_floor_allows(self, passed: bool) -> bool:
        """``passed`` from the legacy canary screen, with the multi-benchmark floor applied.

        WHY THIS EXISTS
            The floor used to be reachable only from :meth:`_reasoned_verdict_decision`, which
            returns None unless DARWINX_GATE_REASONED_VERDICT=1. So a campaign could set
            DARWINX_GATE_MIXTURE_GATE=1, name a three-benchmark spec, log nothing unusual, and evolve
            against one benchmark with no cross-benchmark guard at all -- the exact failure the
            mixture exists to prevent, and invisible because the fallback path is silent by design.

            The floor does not depend on the reasoned verdict for anything. It compares measured
            rates against spec baselines, and it can only downgrade. So it belongs on both paths.

        WHY THIS IS STRICTER THAN THE REASONED-VERDICT PATH
            There, the floor *downgrades*: promote becomes archive, and the commit survives as a
            stepping stone with the reason recorded for the next proposal. That needs a decision with
            more than two values. Here there are two -- keep the iteration or revert it -- so a
            downgrade and a no-op are the same thing, and routing through apply_regression_floor
            would make the floor decorative on this path. A confirmed regression therefore reverts.

            It is a *confirmed* one: the floor escalates a screen-stage drop to the full sample
            before returning passed=False, so this is not a single noisy 25-task read.

        A rejected candidate is not measured: the floor's job is to stop a candidate the in-domain
        signal likes, and paying 75 trials to confirm a rejection is the most expensive way to learn
        nothing. Never raises -- a gate that can crash the loop is worse than no gate.
        """
        if not passed:
            return False
        try:
            ...
            if mx is None:
                return True
            if mx.passed:
                self.log.info("[mixture] floor ok; %s", mx.detail)
                return True
            self.log.info("[mixture] floor regressed %s; reverting the iteration; %s",
                          ", ".join(mx.regressed) or "?", mx.detail)
            return False
        except Exception as exc:  # noqa: BLE001 -- never block the loop
            self.log.warning("[mixture] floor wiring failed (%s); in-domain decision stands", exc)
            return True
    ...
    def _mini_eval_accepts(self, outcome: IterationOutcome) -> bool:
        if self.cfg.pipeline_kind != "regression_resolve":
            ...
            if rv is not None:
                return rv
            return self._mixture_floor_allows(outcome.canary_passed)
    ...
    def _qd_accept_as_specialist(self, outcome: "IterationOutcome") -> bool:
        """QD (DARWINX_GATE_QD_ARCHIVE): keep a variant that CRACKS a claimed task the
    ...
    def _run_iteration(self, i: int) -> IterationOutcome:
        ...
        if self.cfg.guard_enabled:
            ...
            #
            # A PRUNE node is exempt: its whole purpose is a deletion-dominated
            # diff, so the locality scan would reject every prune before it was
            # ever evaluated. The anti-overfitting scan above and the anti-cheat
            # scan below still apply -- a prune may remove accumulated code, but it
            # may not weaken a test or special-case a task name.
            # CONSOLIDATE is exempt for the same reason as PRUNE, plus one more:
            # its diff is a rewrite, so the locality scan's deletion budget would
            # reject the very shape of change it exists to make.
            if _additive_scope_on() and not self._subtractive():
            ...
            try:
                from gate.anti_cheat import scan_diff_for_eval_tampering
        ...
        # After both branches, so the veto ledger sees every guard failure
        # regardless of which gate computed it.
        self._record_guard_veto(outcome.failed_canary_tasks)
        ...
        if _gate_rejects and not self._qd_accept_as_specialist(outcome):
            ...
            if self.cfg.pipeline_kind == "regression_resolve":
                ...
                # The Layer 3 gate is _mini_eval_accepts, which is the reasoned
                # verdict when that is enabled and only falls back to the canary
                # screen otherwise. Reporting every rejection as a canary
                # regression printed "canary regressed: []" for verdict
                # rejections, which is what made mini_smoke_0801b's failure mode
                # unreadable: 8 reverts that all looked like flaky canaries.
                if outcome.failed_canary_tasks:
                    outcome.reason = (
                    ...
                    outcome.reason = (
                        "guard tripped (Layer 3): reasoned verdict rejected the "
                        "change (no canary regressed)"
                    )
            self.guard_violated_ever = True
            self._effort_append(
                f"### Iteration {i}\n\n"
    ...
    # ── automatic guard-reliability tracking (opt-in) ───────────────────────
    # The manual DARWINX_GATE_GUARD_EXCLUDE_TASKS list below fixed the observed case,
    # but it needed a human to notice. This does the same job from evidence: a
    # task that vetoes many DIFFERENT candidates is far more likely to be
    # unreliable than to be genuinely broken by all of them, because those
    # candidates have unrelated diffs. Default OFF -- a guard that stops guarding
    # is a real loss of safety, so this only runs when explicitly enabled.

    def _guard_veto_path(self):
        return self.campaign_dir / "guard_reliability.json"

    def _record_guard_veto(self, failed_guards) -> None:
        """Note which pipelines each guard task has vetoed. Never raises."""
        if not failed_guards or not _guard_autoexclude_on():
            return
        try:
            import json as _json
            path = self._guard_veto_path()
            data = _json.loads(path.read_text()) if path.is_file() else {}
            for task in failed_guards:
                seen = set(data.get(task, []))
                seen.add(str(self.pipeline_id))
                data[task] = sorted(seen)
            path.write_text(_json.dumps(data, indent=1))
        except Exception as exc:  # noqa: BLE001
            self.log.debug("guard veto record skipped: %s", exc)

    def _unreliable_guards(self) -> set[str]:
        """Guard tasks that have vetoed enough DISTINCT pipelines to be suspect."""
        if not _guard_autoexclude_on():
            return set()
        try:
            import json as _json
            path = self._guard_veto_path()
            ...
            need = _guard_autoexclude_threshold()
            return {t for t, pipes in data.items() if len(set(pipes)) >= need}
        except Exception:
            return set()

    @staticmethod
    def _guard_excluded_tasks() -> set[str]:
        """Tasks barred from the regression-guard pool (DARWINX_GATE_GUARD_EXCLUDE_TASKS).

        A guard works by assuming the parent's PASS on that task is reproducible,
        so that a child failing it is evidence of a regression. For a task whose
        true pass-rate is well under 1.0 that assumption is false, and the guard
        manufactures a regression for every candidate regardless of its diff.

        Observed directly on the dev campaign: one task fell 1.0 -> 0.0 under nine
        unrelated candidates -- a six-line prompt removal, a config deletion and a
        77-line loop rewrite alike -- which rejected every behaviour-preserving
        candidate the run produced. Such a task is still worth SCORING; it is only
        unfit to be the instrument that vetoes other work.
        """
        raw = os.environ.get("DARWINX_GATE_GUARD_EXCLUDE_TASKS", "")
    ...
    def _pick_guard_canaries(self, iteration: int, diff_text: str | None = None) -> list[str]:
        ...
        # Evolve path: honor DARWINX_EVAL_GUARD_COUNT to WIDEN the regression panel
        ...
        excluded = self._guard_excluded_tasks() | self._unreliable_guards()
        pool = [t for t in self.passing_for_canary if t not in excluded]
        if excluded and len(pool) != len(self.passing_for_canary):
            self.log.info(
                "guard pool: excluded %d unreliable task(s) -> %d candidates",
                len(self.passing_for_canary) - len(pool), len(pool),
            )
        ...
        candidates = [
            {
                ...
                "claimed_total": len(self.claimed_tasks),
                # Without this the picker is blind to the entire point of a
                # subtractive candidate: its capability numbers are EQUAL to the
                # parent's by design, so on capability alone it always reads as
                # "gained nothing" and gets abandoned.
                "complexity": self._candidate_complexity(out.committed_shas[0]),
                # Measured spend, not measured text. The only axis on which a real
                # harness change produced a resolvable effect (docs/RESULTS_0802.md
                # §2), and one a good consolidation moves while score and line
                # count stay flat. Empty string keeps select.md's block hidden.
                "efficiency": self._candidate_efficiency(out),
                "mini_eval_net_gain": self._mini_eval_net_gain(out),
                "mini_eval_job_dir": (
                    str(out.mini_eval_job_dir) if out.mini_eval_job_dir else None
                ),
                # Graded progress signal (DARWINX_GATE_PROGRESS_SIGNAL=1): the raw
    ...
    def _candidate_efficiency(self, out: "IterationOutcome") -> str:
        ...
        Compared only over tasks the candidate did not fail: a run that bails in
        two calls is cheap for the wrong reason. Off unless
        DARWINX_GATE_EFFICIENCY_SIGNAL is set, because it changes what the picker reads.
        """
        try:
            from . import harness_efficiency as he
            if not he.enabled() or not out.mini_eval_job_dir:
                return ""
            baseline = self._efficiency_baseline_job_dir(out)
            if not baseline:
                return ""
            solved = {t for t, r in out.rewards_per_task.items() if r >= 1.0}
            return he.summary_line(baseline, out.mini_eval_job_dir, solved or None)
        except Exception:
            return ""

    def _efficiency_baseline_job_dir(self, out: "IterationOutcome") -> Path | None:
        """The run to compare this iteration's spend against.

        The most recent kept iteration before this one, else the parent node's
        baseline eval -- the same ladder _iteration_input_logs walks, so the
        comparison tracks whatever the candidate was actually built on top of.
        """
        baseline: Path | None = None
        for prev in self.iterations:
            if prev is out:
                break
            if prev.mini_eval_job_dir and not prev.reverted:
                baseline = prev.mini_eval_job_dir
    ...
    def _candidate_complexity(self, sha: str) -> str:
        """One-line harness size/branch delta of a candidate vs the parent."""
        try:
            from . import harness_complexity as hc
            d = hc.delta(
                str(self.worktree.monet_dir),
                getattr(self.parent_node, "commit_sha", None), sha,
            )
            if not d:
                return "unavailable"
            return (f"{d['d_total_loc']:+d} lines, {d['d_branches']:+d} branches "
                    f"(now {d['after']['total_loc']} lines)")
        except Exception:
            return "unavailable"

    def _picker_subtractive_block(self) -> str:
        """Tell the picker how to read a subtractive candidate, or nothing at all.

        select.md's standing rule is "pick the parent when no candidate's reward
        AND pass-rate beat it". For a prune or a consolidation those are EQUAL to
        the parent whenever it succeeded, so that rule abandons every successful
        one -- which is exactly what happened to a consolidation the judge had
        already PROMOTEd. Returns "" for ordinary nodes so their prompt is
        unchanged.
        """
        try:
            ...
            # Prompt decoration must never be able to break commit selection.
            return ""
        kind = "CONSOLIDATE (rewrite for simplicity)" if consolidate else "PRUNE (subtraction)"
        return (
            f"\n## THIS IS A {kind} NODE — the selection rule below is INVERTED\n\n"
            "This node's goal was NOT to raise the score. It was to hold capability "
            "while making the harness smaller or structurally simpler. So:\n\n"
            "- **Equal claimed rewards and pass-rates vs the parent is SUCCESS here, "
            "not a no-op.** Each candidate's `complexity` line reports its harness "
            "size and branch-count change against the parent.\n"
            "- **Pick the candidate that preserves capability with the LARGEST real "
            "simplification** (most lines removed, or most branches removed — a "
            "branch reduction at similar line count is the stronger result, since "
            "folding several special cases into one general path is the intended "
            "move).\n"
            "- **Pick the parent ONLY if** every candidate either regressed a "
            "claimed task or a canary, or simplified nothing (complexity change "
            "of roughly zero, or positive). Do NOT pick the parent merely because "
            "no candidate improved the score — improving the score was not the "
            "job.\n"
        )
    ...
    def _invoke_picker_agent(self, candidates: list[dict[str, Any]]) -> str | None:
        ...
        prompt = cursor_agent.render_prompt(
            self._template_path("select.md"),
            {
                ...
                "subset_label": self.cfg.subset_label,
                "subtractive_block": self._picker_subtractive_block(),
    ...
    def _lineage_depth(self) -> int:
        """How many accepted nodes separate the parent from the campaign root."""
        depth = 0
        node = self.parent_node
        seen: set[str] = set()
    ...
    def _is_consolidate_node(self) -> bool:
        ...
        if decided:
            self.log.info(
                ...
                "capability held + complexity reduced)"
            )
        return decided

    def _simplification_win(self, removed: int, added: int) -> bool:
        """Did this behaviour-preserving candidate actually simplify the harness?

        A PRUNE is judged on raw line count: deleting is its whole job.

        A CONSOLIDATE cannot be, because the rewrite it exists to make is often
        line-neutral -- three special-case branches folded into one general
        mechanism can add as many lines as it removes while being unambiguously
        simpler. So it is judged on measured complexity, and a drop in BRANCHES
        counts even when the line count does not move. If complexity cannot be
        measured we fall back to the line-count rule rather than accept on faith.
        """
        if not self._is_consolidate_node():
            return removed > added
        try:
            from . import harness_complexity as hc
            d = hc.delta(
                str(self.worktree.monet_dir),
                getattr(self.parent_node, "commit_sha", None),
                worktree.head_sha(self.worktree.monet_dir),
            )
        except Exception:
            d = {}
        if not d:
    ...
    def _subtractive(self) -> bool:
        ...
        return self._is_prune_node() or self._is_consolidate_node()

    def _complexity_evidence(self) -> str:
        """Harness size/branching before vs after, for the verdict. Never raises."""
        try:
            from . import harness_complexity as hc
            parent_sha = getattr(self.parent_node, "commit_sha", None)
            child_sha = worktree.head_sha(self.worktree.monet_dir)
            return hc.render_for_prompt(
                hc.delta(str(self.worktree.monet_dir), parent_sha, child_sha)
            )
        except Exception:
            return ""

    def _is_prune_node(self) -> bool:
        """Whether this pipeline is a PRUNE (subtraction) node.

        Drawn locally from a hash of the pipeline id rather than scheduled by the
        supervisor. Each worker draws independently, the campaign-wide fraction
        converges on ``DARWINX_GATE_PRUNE_RATE``, and the CLI / worker-spawn surface is
        untouched -- a prune node is an ordinary evolve worker that reads a
        different prompt and is judged by a different rule.

        Suppressed near the root, where a clean base-rooted campaign has nothing
        accumulated to remove.
        """
        if self._prune_node is not None:
        ...
        try:
            if _prune_enabled() and _prune_rate() > 0.0:
                ...
                if depth >= _prune_min_lineage():
                    import hashlib
                    digest = hashlib.sha1(f"prune:{self.pipeline_id}".encode()).digest()
                    draw = int.from_bytes(digest[:4], "big") / 0xFFFFFFFF
                    decided = draw < _prune_rate()
                else:
                    self.log.debug(
                        "prune lottery skipped: lineage depth %d < %d",
                        depth, _prune_min_lineage(),
                    )
        except Exception as exc:  # noqa: BLE001 — never block the loop
            self.log.debug("prune lottery skipped (%s); treating as evolve node", exc)
            decided = False
        ...
        if decided:
            self.log.info(
                ...
                _prune_rate(), _prune_min_lineage(),
            )
        return decided

    def _prune_dossier_block(self) -> str:
        """Inventory of what this lineage ACCUMULATED, for the prune proposer.

        A prune proposer cannot act without this: it has to know which parts of
        the agent are this campaign's evolution artifacts (fair game) and which
        are stock product behavior (off limits). Built by diffing the parent
        against the campaign ROOT -- everything in that diff was added by
        evolution, by definition.

        Returns "" for non-prune nodes so the additive prompt is unchanged, and on
        any failure, so a dossier problem degrades the prune into a no-target node
        rather than breaking the run.
        """
        # A CONSOLIDATE node needs this even more than a PRUNE: the dossier is the
        # list of accumulated modifications that are candidates for being folded
        # together, and without it the rewrite has to rediscover the lineage.
        if not self._subtractive():
            return ""
        try:
            import subprocess

            monet_dir = self._worktree_monet_dir()
            if not (monet_dir and self.parent_node and self.parent_node.commit_sha):
                return ""
            root = self.parent_node
            seen: set[str] = set()
            while root is not None and root.parent_id and root.id not in seen:
                seen.add(root.id)
                nxt = tree.get_node(self.conn, root.parent_id)
                if nxt is None:
                    break
                root = nxt
            root_sha = root.commit_sha if root else None
            if not root_sha or root_sha == self.parent_node.commit_sha:
                return ""

            def _git(*args: str) -> str:
                res = subprocess.run(
                    ["git", "-C", str(monet_dir), *args],
                    capture_output=True, text=True, timeout=30,
                )
            ...
            out.append(
                f"Campaign root: `{worktree.short_sha(root_sha)}` (the clean base agent).\n"
                f"Parent under review: `{worktree.short_sha(self.parent_node.commit_sha)}`.\n"
                "Everything below was ADDED by evolution and is therefore a legitimate "
                "prune target. Anything NOT below is stock product behavior — leave it alone.\n"
            )
            if commits:
                out.append("### Lineage commits (newest first)\n```text\n" + commits[:4000] + "\n```\n")
            if stat:
                out.append("### Cumulative diff vs base\n```text\n" + stat[-2500:] + "\n```\n")
            if added_skills:
                out.append(
                    "### Skill-registry entries this lineage added\n"
                    + "\n".join(f"- `{s}`" for s in dict.fromkeys(added_skills))
            ...
            if solved:
                out.append(
                    "### Currently SOLVED tasks — removal must not regress these\n```text\n"
                    + ", ".join(solved[:60]) + "\n```\n"
                )
            return "".join(out)
        except Exception as exc:  # noqa: BLE001 — advisory context, never fatal
            self.log.warning("prune dossier failed (ignored): %s", exc)
            return ""
    ...
    def _render_analyze_prompt(self, i: int, input_logs: Path) -> str:
        ...
        if self.cfg.pipeline_kind == "regression_resolve":
            template = "regression_analyze.md"
        elif self._is_consolidate_node():
            # Rewrite node: both analyze.md (forbids deletion) and prune_analyze.md
            # (forbids touching pre-evolve code, demands a deletion-dominated diff)
            # would forbid what this node exists to do.
            template = "consolidate_analyze.md"
        elif self._is_prune_node():
            # Subtraction node: analyze.md forbids deletion outright, so a prune
            # must not read it.
            template = "prune_analyze.md"
        else:
            template = "analyze.md"
    ...
    def _prompt_context(
        ...
        *,
        ...
        return {
            ...
            # Empty string unless DARWINX_GATE_PRESERVE_EXTEND=1, so the vanilla
            ...
            # Empty unless DARWINX_GATE_CONTRACT_GUIDED=1. Never raises.
            ...
            # unless DARWINX_GATE_BESTOF2_CONTRAST=1. Never raises.
            ...
            # the tree's shared memory. Empty unless DARWINX_GATE_COLLECTIVE_KNOWLEDGE=1.
            ...
            # DARWINX_GATE_REASONED_VERDICT=1. Never raises.
            "recent_lessons_block": self._recent_lessons_block(),
            # Subtraction: the inventory of modifications this lineage accumulated
            # on top of the clean base, so a PRUNE node knows what is fair game.
            # Empty unless this node drew the prune lottery.
            "prune_dossier_block": self._prune_dossier_block(),
        }
    ...
    def _trace_qc_summary(self, trials: list[dict] | None, eval_dir: str | None) -> str:
        ...
        Ablation switch: ``DARWINX_EVOLVE_TRACE_QC=0`` disables the injection so the
        analyze prompt is byte-identical to a no-trace-analyzer run (the control
        arm). Defaults on.
        """
        if os.environ.get("DARWINX_EVOLVE_TRACE_QC", "1").strip().lower() in ("0", "false", "no", "off"):
            return ""
        if not trials:
            return ""
        try:
    ...
    def _collective_knowledge_block(self) -> str:
        ...
        evolving in isolation. Empty unless DARWINX_GATE_COLLECTIVE_KNOWLEDGE=1.
        Fail-safe: never raises into the loop.
        """
        if os.environ.get("DARWINX_GATE_COLLECTIVE_KNOWLEDGE", "0").strip().lower() not in {
            "1", "true", "yes", "on",
        }:
            return ""
        try:
    ...
    def _collective_knowledge_for_gate(self) -> str | None:
        ...
        (DARWINX_GATE_KNOWLEDGE_GATE) — same aggregation as the proposer block but
        returned raw so it can be threaded into the reasoned verdict's evidence.
        Independent of the proposer-side DARWINX_GATE_COLLECTIVE_KNOWLEDGE flag so the
    ...
    def _task_contract_block(self) -> str:
        ...
        Gated by ``DARWINX_GATE_CONTRACT_GUIDED`` (default on). Read-only, bounded,
        never raises (degrades to "").
        """
        try:
            if os.environ.get("DARWINX_GATE_CONTRACT_GUIDED", "1").strip().lower() in {
                "0", "false", "no", "off"
            }:
                return ""
            dataset = (
    ...
    def _preserve_extend_block(self) -> str:
        ...
        DARWINX_GATE_PRESERVE_EXTEND is off, keeping the control arm identical to
        plain exp_05. Never raises (degrades to "").
        """
        try:
            if os.environ.get("DARWINX_GATE_PRESERVE_EXTEND", "0").strip() != "1":
                return ""
            # This contract says "not a free-form rewrite" and names prior edits as
            # invariants -- which is precisely what a CONSOLIDATE node must be free
            # to restructure. Capability is still protected: its verdict rule
            # rejects any regression, and the canary guards still run.
            if self._is_consolidate_node():
                return ""
            assert self.parent_node
    ...
    def _run_pre_final_equivalence_gate(self):
        ...
        """
        if not gate_hook.is_equivalence_gate_enabled():
            return None
        ...
        if os.environ.get("DARWINX_GATE_ANTI_CHEAT_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}:
            try:
                from gate.anti_cheat import scan_diff_for_eval_tampering
            ...
            if cheat_reasons:
                from gate import matchfix_gate as _mfg
                self.log.info(
        ...
        eq_ctx = gate_hook.EquivalenceGateContext(
            pipeline_id=self.pipeline_id,
            campaign=self.cfg.campaign,
        ...
        try:
            with self._heartbeat_during_eval("equivalence gate"):
                return gate_hook.run_equivalence_gate(eq_ctx)
        except Exception as e:  # noqa: BLE001
    ...
    def _should_archive_stepping_stone(self, eq_verdict) -> bool:
        ...
        regressions still hard-reject. No-op unless DARWINX_GATE_HYBRID_ARCHIVE=1.
        """
    ...
    def _reject_via_equivalence_gate(self, eq_verdict) -> int:
        ...
        try:
            ...
            gate_hook.mark_ltm_source_rejected(
    ...
    def _finalize(self) -> int:
        ...
        if deferred_node_eval:
            if _fixed_eval_panel():
                # Shared panel: same tasks, same denominator, every node. A node
                # can no longer raise its score by claiming fewer tasks.
                final_tasks = _build_fixed_panel(
                    list(self.cfg.subset_tasks), self.cfg.campaign,
                )
                self.log.info(
                    "fixed-panel: scoring node on the shared %d-task panel "
                    "(digest=%s)",
                    len(final_tasks), _panel_digest(final_tasks),
                )
            else:
        ...
        node_eval = self._persist_node_eval_result(
            ...
            metadata={
                ...
                "panel_digest": _panel_digest(list(final_tasks or [])),
                "panel_size": len(final_tasks or []),
                "fixed_panel": _fixed_eval_panel(),
            },
        )
        ...
        # DARWINX_GATE_REQUIRE_EXTENSION=0 (off => pure exp_05 / DGM behavior).
        require_extension = (
            os.environ.get("DARWINX_GATE_REQUIRE_EXTENSION", "0").strip() == "1"
        )
        n_extend = len(improved_tasks)
        n_regress = len(regressed_tasks)
        # A PRUNE node is exempt: it removes code rather than solving new tasks, so
        # it extends nothing by construction and this contract would reject every
        # successful subtraction.
        if (
            require_extension
            and self.cfg.pipeline_kind != "regression_resolve"
            and not self._subtractive()
        ):
        ...
        # ─── Successful PRUNE: behavior preserved, agent smaller ──────────
        # This is the carve-out that makes subtraction possible at all. A clean
        # removal is behavior-preserving BY DESIGN, so it lands with improved=0
        # and regressed=0 -- exactly the shape the phantom guard below treats as a
        # worthless no-op and resets to the parent. Under the additive-only
        # pipeline that made a neutral prune unrepresentable: it could never be
        # kept, so nothing was ever removed and the agent could only grow.
        #
        # Here a prune that preserved every task AND net-removed lines is recorded
        # as a real `completed` node with its own commit and score, so later nodes
        # can be built on the simplified agent. It does not need a score gain to
        # be worth keeping -- less surface area is the win. A prune that regressed
        # something falls through to the normal paths below and is judged there,
        # which is what stops us from deleting load-bearing behavior.
        if (
            self.cfg.pipeline_kind != "regression_resolve"
            and self._subtractive()
            and not improved_tasks
            and not regressed_tasks
        ):
            removed, added = self._diff_line_counts()
            if self._simplification_win(removed, added):
                latest = worktree.head_sha(self.worktree.monet_dir)
                self.log.info(
                    "%s accepted: child %s preserved every task (improved=0 "
                    "regressed=0) while net-removing %d lines (-%d/+%d) — kept as "
                    "completed node (commit %s, score %.4f)",
                    "CONSOLIDATE" if self._is_consolidate_node() else "PRUNE",
                    self.child_node.id, removed - added, removed, added,
                    (latest or "")[:12],
                ...
                tree.update_node(
                    ...
                    resolved_tasks_json=resolved_tasks + [{
                        ...
                        "lines_added": added,
                        "net_lines_removed": removed - added,
                    }],
                    claimed_task_scores_json=per_task,
                    status="completed",
                )
                self.child_node = tree.get_node(self.conn, self.child_node.id)
                self._push_child_branch()
                self._mark_done("completed")
                return 0
            self.log.info(
                "PRUNE node %s removed nothing (-%d/+%d) — falling through to the "
                "phantom guard; a prune that does not shrink the agent is a no-op",
                self.child_node.id, removed, added,
            )
        ...
        # DARWINX_GATE_QD_ARCHIVE keep it as `archived` with its REAL solved/improved/
...
class FullSetEvalPipeline:
    ...
    def _run_full(self, **kwargs):
        ...
        task list is known and ``DARWINX_EVAL_FULLSET_METRIC=avg``, with whole-job
    ...
    def run(self) -> int:
        try:
            ...
            # empty list, unlike run_full's []=all). When DARWINX_EVAL_FULLSET_METRIC
            # =avg and DARWINX_EVAL_FULLSET_TASKS is provided (comma-sep), pass it so
            # _run_full routes to run_full_avg_k (leaderboard avg@k). Otherwise []
            # = full best-of-N sweep, unchanged.
            _fs_metric = os.environ.get("DARWINX_EVAL_FULLSET_METRIC", "").strip().lower()
            _fs_tasks: list[str] = []
            if _fs_metric == "avg":
                _raw = os.environ.get("DARWINX_EVAL_FULLSET_TASKS", "").strip()
