...
package works from any clone. ``WORKTREE_PARENT`` defaults to
``eval_worktrees/`` next to the eval repo. Override either with
environment variables:

    DARWINX_EVAL_REPO_ROOT       absolute path to monet_code_eval clone
    DARWINX_EVAL_WORKTREE_PARENT absolute dir to put per-pipeline worktrees in
"""
...
def _default_worktree_parent() -> Path:
    """Where per-pipeline worktrees are created.

    Defaults to ``<repo>/../eval_worktrees`` rather than the repo's parent
    itself: the bare parent scatters hundreds of ``monet_code_eval__*`` dirs
    over whatever directory the checkout happens to sit in.
    """
    env = os.environ.get("DARWINX_EVAL_WORKTREE_PARENT")
    if env:
        return Path(env).expanduser().resolve()
    return _default_repo_root().parent / "eval_worktrees"


# Directory (relative to REPO_ROOT / a worktree) holding the agent source the
# loop mutates. Defaults to monet_code so existing campaigns are unaffected;
# set DARWINX_EVOLVE_AGENT_SUBMODULE to evolve a different agent, e.g.
# agents/mini_swe_agent/vendor. Must be a REGISTERED submodule -- the branch
# machinery below drives `git submodule` commands against it.
AGENT_SUBMODULE = os.environ.get("DARWINX_EVOLVE_AGENT_SUBMODULE", "monet_code").strip() or "monet_code"
...
class Worktree:
    eval_dir: Path        # WORKTREE_PARENT / monet_code_eval__evolve_<branch>
...
def make_worktree_path(branch: str) -> Path:
    """Sanitize a branch name into a worktree directory under ``WORKTREE_PARENT``."""
...
def current_monet_branch() -> str:
    ...
    monet_dir = REPO_ROOT / AGENT_SUBMODULE
...
def resolve_seed_commit(
    ...
    Used when ``$DARWINX_EVOLVE_ROOT_COMMIT`` is set to evolve on top of a
    ...
    if not sha:
        raise RuntimeError(
            f"could not resolve DARWINX_EVOLVE_ROOT_COMMIT={ref!r} in {monet_dir}. "
            f"Ensure the canonical monet_code clone can reach it — e.g. "
            f"`git -C {monet_dir} fetch origin <branch-containing-the-commit>` "
            f"(set DARWINX_EVOLVE_ROOT_FETCH_REF to that branch), then retry."
        )
...
def add_eval_worktree(
    ...
    *it* per pipeline. The eval-side ``eval_dir`` is a plain directory that
    ...
    """
    canonical = repo_root / AGENT_SUBMODULE
    if not (canonical / ".git").exists():
        raise RuntimeError(
            ...
            f"it first, e.g.\n"
            f"    git clone https://github.com/your-org/monet_code.git "
            f"{canonical}"
        )
    ...
    monet_dir = wt_dir / AGENT_SUBMODULE
...
def link_dotenv_into_worktree(wt_dir: Path, *, repo_root: Path = REPO_ROOT) -> bool:
    ...
    without `DARWINX_EVAL_REPO_ROOT` set) that path doesn't exist, and
...
def remove_eval_worktree(wt_dir: Path, *, repo_root: Path = REPO_ROOT) -> bool:
    ...
    """
    wt_dir = Path(wt_dir)
    canonical = Path(repo_root) / AGENT_SUBMODULE
    monet_dir = wt_dir / AGENT_SUBMODULE
...
def delete_eval_branch(branch: str, *, repo_root: Path = REPO_ROOT) -> bool:
    ...
    proc = _run(
        ["git", "branch", "-D", branch],
        cwd=Path(repo_root) / AGENT_SUBMODULE,
        check=False,
...
def delete_submodule_branch(
    ...
    *,
    ...
    monet_canonical = repo_root / AGENT_SUBMODULE
    proc = _run(
        ["git", "branch", "-D", branch],
        cwd=monet_canonical,
        check=False,
