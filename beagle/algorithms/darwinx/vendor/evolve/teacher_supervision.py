...
STATUS
------
Prototype. DISABLED by default. Enable with ``DARWINX_GATE_TEACHER_SUPERVISION=1``.
...
``DARWINX_GATE_TEACHER_SUPERVISION=1``; it is also wrapped in try/except so any failure
...
ENV_FLAG = "DARWINX_GATE_TEACHER_SUPERVISION"
...
TEACHER_CONFIG = os.environ.get(
    "DARWINX_GATE_TEACHER_CONFIG",
    "configs/self_evolve/teacher_cursor_solver_tb21.yaml",
)
# Where teacher rollouts are cached between runs. Deployment-specific, so the fallback is a
# temp dir rather than any particular cluster's scratch mount; point the env at durable storage
# if you want the cache to survive across machines.
TEACHER_RESULTS_ROOT = os.environ.get(
    "DARWINX_GATE_TEACHER_RESULTS_ROOT",
    os.path.join(tempfile.gettempdir(), "darwinx_teacher_rollouts"),
)
TEACHER_TIMEOUT_S = int(os.environ.get("DARWINX_GATE_TEACHER_TIMEOUT_S", "5400"))
...
def maybe_teacher_block(task: str, trials: Any = None, eval_dir: Any = None) -> str:
    """Strict no-op unless DARWINX_GATE_TEACHER_SUPERVISION=1. Returns the contrastive
