...
STATUS
------
Prototype. DISABLED by default. Enable with ``ATELIER_SELF_CONTRAST=1``. Cache-only +
...
Strict no-op unless ``ATELIER_SELF_CONTRAST=1``; wrapped so any failure degrades to
the failure-only baseline.

CONFIG
------
- ``ATELIER_SELF_CONTRAST``         : "1" to enable.
- ``ATELIER_SELF_CONTRAST_SOURCES`` : comma-separated run dirs (or globs) to mine for
...
ENV_FLAG = "ATELIER_SELF_CONTRAST"

_DEFAULT_SOURCES = (
    "/opt/sagemaker/tmp/avg5_lsponly/runs/*/,"
    "/opt/sagemaker/tmp/pa_clean7/runs/*/,"
    "/opt/sagemaker/tmp/avg5_lsponly_clean7/runs/*/,"
    "/opt/sagemaker/tmp/avg5_lsponly_clean5b/runs/*/"
)
...
def _sources() -> list[str]:
    raw = os.environ.get("ATELIER_SELF_CONTRAST_SOURCES", _DEFAULT_SOURCES)
    out: list[str] = []
...
def maybe_self_contrast_block(task: str, trials: Any = None, eval_dir: Any = None) -> str:
    """Strict no-op unless ATELIER_SELF_CONTRAST=1. Returns the self-contrast block, or
