...
def unresolved_tasks(
    ...
    *,
    ...
    # avg@5-derived variance list (MONET_EVAL_VARIANCE_TASKS) so the campaign
    ...
    candidates = list(dict.fromkeys(list(variance) + list(failed)))
    if subset_filter is not None:
        allowed = set(subset_filter)
        candidates = [t for t in candidates if t in allowed]
    # Operator denylist (MONET_EVAL_EXCLUDE_TASKS): hard-remove tasks that can
...
def pick_and_claim(
    ...
    *,
    ...
    # MONET_EVAL_PRIORITY_TASKS to a comma-separated list of task-name
    ...
    # Cluster-batch claiming (MONET_EVAL_CLUSTER_CLAIM=1): draw the whole batch
...
def _cluster_claim_on() -> bool:
    ...
    (cluster-batch evolution). Gated by ``MONET_EVAL_CLUSTER_CLAIM``."""
    return os.environ.get("MONET_EVAL_CLUSTER_CLAIM", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
# ``MONET_EVAL_CLUSTERS`` (JSON {name: [substrings]}). Tasks matching no cluster
# fall in "misc".
...
def _capability_clusters() -> dict[str, list[str]]:
    raw = os.environ.get("MONET_EVAL_CLUSTERS", "").strip()
    if raw:
        try:
            import json
            d = json.loads(raw)
...
def _priority_substrings() -> list[str]:
    ...
    Read from ``MONET_EVAL_PRIORITY_TASKS`` (comma-separated). Empty/unset
    disables cluster targeting (prior curriculum ordering is used verbatim).
    """
    raw = os.environ.get("MONET_EVAL_PRIORITY_TASKS", "")
...
def _variance_tasks() -> list[str]:
    ...
    ``MONET_EVAL_VARIANCE_TASKS`` (comma-separated). These are unioned into the
    ...
    raw = os.environ.get("MONET_EVAL_VARIANCE_TASKS", "")
...
def _excluded_substrings() -> list[str]:
    ...
    Read from ``MONET_EVAL_EXCLUDE_TASKS`` (comma-separated, substring match).
    ...
    """
    raw = os.environ.get("MONET_EVAL_EXCLUDE_TASKS", "")
    return [s.strip().lower() for s in raw.split(",") if s.strip()]
...
def _variance_band_enabled() -> bool:
    ...
    Gated by ``MONET_EVAL_CLAIM_VARIANCE_BAND`` (``1``/``true``/``yes`` → on).
    Default off so shared callers and other campaigns are unaffected.
    """
    return os.environ.get("MONET_EVAL_CLAIM_VARIANCE_BAND", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
