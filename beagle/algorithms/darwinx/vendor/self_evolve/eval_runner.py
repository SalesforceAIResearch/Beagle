    ...
    *,
    ...
    Args:
        ...
        job_name: optional --job-name override; otherwise harbor uses a
            timestamp.
        extra_env: extra MONET_EVAL_* / harbor env vars to merge into the
...
def _harbor_post_result_grace_s() -> float:
    raw = os.environ.get("MONET_EVAL_HARBOR_POST_RESULT_GRACE_S", "120") or "120"
    try:
...
def prune_stale_docker_resources_if_idle(*, reason: str = "") -> bool:
    ...
    """
    if os.environ.get("MONET_EVAL_SKIP_DOCKER_PRUNE") == "1":
        return False
