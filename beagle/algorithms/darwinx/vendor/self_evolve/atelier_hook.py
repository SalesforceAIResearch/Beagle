...
- ``maybe_blend_score(ctx, base_score)`` — when ``ATELIER_FITNESS_ALPHA``
...
- ``on_node_finalized(ctx)`` — when ``ATELIER_GATE_ENABLED=1``, runs the
...
- ``ATELIER_GATE_ENABLED`` (``0`` / ``1``, default ``0``) — record per-node
  gate decisions.
- ``ATELIER_FITNESS_ALPHA`` (float in [0, 1], default ``0``) — blending
...
- ``ATELIER_SCOPE_MODE`` (``soft_flag`` / ``strict_reject``, default
  ``soft_flag``) — Layer-4 scope filter mode.
- ``ATELIER_REPORTS_SUBDIR`` (default ``atelier``) — subdirectory under
  ``reports/<campaign>/`` to write decision sidecars into.
- ``ATELIER_PREDICTIONS_ENABLED`` (``0`` / ``1``, default ``1``) — if
  ``1``, ``capture_review_prediction`` parses the proposer's review
  text for a ``predicted_impact`` YAML block and writes a per-node
  sidecar. Independent of ``ATELIER_GATE_ENABLED`` — predictions are
...
logger = logging.getLogger("self_evolve.atelier_hook")
...
def is_gate_enabled() -> bool:
    ...
    return os.environ.get("ATELIER_GATE_ENABLED", "0").strip() == "1"


def fitness_alpha() -> float:
    """Atelier fitness-blend alpha, parsed from env. 0 = disabled."""
    raw = os.environ.get("ATELIER_FITNESS_ALPHA", "0").strip()
    try:
        val = float(raw)
    except ValueError:
        logger.warning(
            "ATELIER_FITNESS_ALPHA=%r is not a valid float; treating as 0", raw
        )
        return 0.0
    if not 0.0 <= val <= 1.0:
        logger.warning(
            "ATELIER_FITNESS_ALPHA=%r is outside [0, 1]; clamping to 0", raw
        )
        return 0.0
    return val
...
def is_predictions_enabled() -> bool:
    ...
    """
    return os.environ.get("ATELIER_PREDICTIONS_ENABLED", "1").strip() == "1"
...
def is_equivalence_gate_enabled() -> bool:
    ...
    ``ATELIER_EQUIVALENCE_GATE_ENABLED=1`` for E4+ campaigns once the
    retrospective on E3's stored candidates confirms the gate would
    have rejected the catastrophes.
    """
    return os.environ.get("ATELIER_EQUIVALENCE_GATE_ENABLED", "0").strip() == "1"
...
def equivalence_probe_k() -> int:
    ...
    via ``ATELIER_EQUIVALENCE_PROBE_K``.
    """
    raw = os.environ.get("ATELIER_EQUIVALENCE_PROBE_K", "6").strip()
    try:
        val = int(raw)
    except ValueError:
        logger.warning(
            "ATELIER_EQUIVALENCE_PROBE_K=%r is not an int; using 6", raw
        )
        return 6
    return max(1, val)
...
def equivalence_n_adversarial() -> int:
    ...
    """
    raw = os.environ.get("ATELIER_EQUIVALENCE_N_ADVERSARIAL", "3").strip()
    try:
        val = int(raw)
    except ValueError:
        logger.warning(
            "ATELIER_EQUIVALENCE_N_ADVERSARIAL=%r is not an int; using 3", raw
        )
        return 3
    return max(0, val)
...
def is_sibling_pool_enabled() -> bool:
    ...
    high-signal. Disable with ``ATELIER_SIBLING_POOL_ENABLED=0``
    for ablation runs.
    """
    return (
        os.environ.get("ATELIER_SIBLING_POOL_ENABLED", "1").strip() == "1"
    )
...
def sibling_pool_k() -> int:
    ...
    """
    raw = os.environ.get("ATELIER_SIBLING_POOL_K", "2").strip()
    try:
        val = int(raw)
    except ValueError:
        logger.warning(
            "ATELIER_SIBLING_POOL_K=%r is not int; using 2", raw
        )
        return 2
    return max(1, val)
...
def sibling_pool_novelty_m() -> int:
    ...
    """
    raw = os.environ.get("ATELIER_NOVELTY_M", "4").strip()
    try:
...
def equivalence_probe_k_fixed() -> bool:
    """If True, use the env-fixed ``ATELIER_EQUIVALENCE_PROBE_K`` as
    ...
    return (
        os.environ.get(
            "ATELIER_EQUIVALENCE_PROBE_K_FIXED", "0"
        ).strip() == "1"
    )
...
def equivalence_n_votes() -> int:
    ...
    """
    raw = os.environ.get("ATELIER_EQUIVALENCE_N_VOTES", "3").strip()
    try:
        val = int(raw)
    except ValueError:
        logger.warning(
            "ATELIER_EQUIVALENCE_N_VOTES=%r is not an int; using 3", raw
        )
        return 3
    return max(1, val)
...
def equivalence_require_extension() -> bool:
    ...
    until AFTER final-eval). Set ``ATELIER_EQUIVALENCE_REQUIRE_EXTENSION=1``
    when wiring the post-final-eval extension check.
    """
    return (
        os.environ.get("ATELIER_EQUIVALENCE_REQUIRE_EXTENSION", "0").strip()
        == "1"
    )
...
def equivalence_model() -> str:
    ...
    return os.environ.get(
        "ATELIER_EQUIVALENCE_MODEL", "gpt-5.4-mini"
    ).strip()


def equivalence_provider() -> str:
    return os.environ.get(
        "ATELIER_EQUIVALENCE_PROVIDER", "sfr_gateway"
    ).strip()


def scope_mode_str() -> str:
    return os.environ.get("ATELIER_SCOPE_MODE", "soft_flag").strip()


def reports_subdir() -> str:
    return os.environ.get("ATELIER_REPORTS_SUBDIR", "atelier").strip()
...
def is_ltm_enabled() -> bool:
    ...
    the proposer brain. Opt out via ``ATELIER_LTM_ENABLED=0``.
    """
    return os.environ.get("ATELIER_LTM_ENABLED", "1").strip() == "1"


def ltm_max_entries() -> int:
    """Cap on LTM file size + prompt-view depth (default 30)."""
    raw = os.environ.get("ATELIER_LTM_MAX_ENTRIES", "30").strip()
    try:
        val = int(raw)
    except ValueError:
        logger.warning(
            "ATELIER_LTM_MAX_ENTRIES=%r is not an int; using 30", raw
        )
        return 30
    if val < 1:
        return 1
...
class EquivalenceGateContext:
    ...
    ATELIER_EQUIVALENCE_REQUIRE_EXTENSION=1)."""
...
class HookContext:
    ...
    reports_root: Path = field(default=Path("reports"))
    monet_dir: Path = field(default=Path("monet_code"))
...
def verifier_model() -> str:
    ...
    return os.environ.get("ATELIER_VERIFIER_MODEL", "gpt-5.4-mini").strip()


def verifier_provider() -> str:
    """Credential provider for the verifier backend."""
    return os.environ.get("ATELIER_VERIFIER_PROVIDER", "sfr_gateway").strip()
...
def verifier_criteria_profile() -> str:
    ...
    """
    return os.environ.get("ATELIER_VERIFIER_CRITERIA_PROFILE", "tb2").strip()
...
def maybe_blend_score(ctx: HookContext, *, base_score: float) -> float:
    ...
    Returns the blended score when ``ATELIER_FITNESS_ALPHA > 0`` and all
    ...
    if ctx.final_eval_job_dir is None or not Path(ctx.final_eval_job_dir).is_dir():
        logger.warning(
            "ATELIER_FITNESS_ALPHA=%.2f set but no final_eval_job_dir on the hook "
    ...
    try:
        from atelier import trajectory_loader, verifier as v_mod
        from atelier import verifier_backend, verifier_fitness
    except ImportError as e:
        logger.warning(
            "ATELIER_FITNESS_ALPHA=%.2f set but atelier package missing a dep: %s "
    ...
    # the phantom filter. Env ATELIER_VERIFIER_SGV=1; default = legacy template.
    _sgv_on = os.environ.get("ATELIER_VERIFIER_SGV", "0").strip().lower() in {
        "1", "true", "yes", "on",
    }
...
def _git_diff_files(
    *, monet_dir: Path, parent_commit: str | None, child_commit: str | None
    ...
    for the same shape; we inline it here to keep atelier_hook
...
def _run_gate_and_serialize(ctx: HookContext) -> dict[str, Any] | None:
    ...
    try:
        from atelier import gate, scope_filter
    except ImportError as e:
        logger.warning(
            "ATELIER_GATE_ENABLED=1 but atelier package not importable: %s",
            e,
        )
        return None
    ...
    try:
        ...
        logger.warning(
            "ATELIER_SCOPE_MODE=%r is not a valid mode; using soft_flag",
            scope_mode_str(),
        )
...
def on_node_finalized(ctx: HookContext) -> None:
    """If ``ATELIER_GATE_ENABLED=1``, run the gate and write a sidecar
...
_PROMPT_DIR = Path(__file__).resolve().parent.parent / "atelier" / "prompts"
...
def equivalence_reprobe_enabled() -> bool:
    ...
    disable with ATELIER_EQUIVALENCE_REPROBE=0.
    """
    return os.environ.get("ATELIER_EQUIVALENCE_REPROBE", "1").strip() == "1"
...
def probe_k_samples() -> int:
    ...
    restore the legacy single-sample probe. Override with ATELIER_PROBE_K_SAMPLES.
    """
    raw = os.environ.get("ATELIER_PROBE_K_SAMPLES", "3").strip()
    try:
        return max(1, int(raw))
    except ValueError:
        return 3
...
def run_equivalence_gate(eq_ctx: "EquivalenceGateContext"):
    ...
    """
    try:
        from atelier import matchfix_gate
    except ImportError as e:
        logger.warning(
            "ATELIER_EQUIVALENCE_GATE_ENABLED=1 but atelier.matchfix_gate "
    ...
    # Replaces the v5/v7 fixed ATELIER_EQUIVALENCE_PROBE_K=6 with an
    # LLM-recommended K based on risk_level + modified_surfaces. Opt
    # out via ATELIER_EQUIVALENCE_PROBE_K_FIXED=1.
    ...
    if equivalence_probe_k_fixed():
        ...
        try:
            from atelier import coverage_sizer
...
def capture_review_prediction(
    *,
    ...
    Default-on (``ATELIER_PREDICTIONS_ENABLED=1``). Disabled-off via
    env. Never raises.
    """
    ...
    try:
        from atelier import predictions
    except ImportError as e:
        logger.warning(
            "ATELIER_PREDICTIONS_ENABLED=1 but atelier.predictions not "
...
def is_trace_digest_enabled() -> bool:
    ...
    cheap. Opt out via ``ATELIER_TRACE_DIGEST_ENABLED=0``.
    """
    return os.environ.get("ATELIER_TRACE_DIGEST_ENABLED", "1").strip() == "1"
...
def render_trial_digests(
    *,
    ...
    try:
        from atelier import trace_analyzer
    except ImportError as e:
        logger.warning(
            "ATELIER_TRACE_DIGEST_ENABLED=1 but trace_analyzer not "
            "importable (%s) — skipping",
            e,
        )
        return ""
...
def render_long_term_memory(
    *, reports_root: Path, campaign: str, max_entries: int | None = None
    ...
    try:
        ...
        logger.warning(
            "ATELIER_LTM_ENABLED=1 but atelier.long_term_memory not "
            "importable (%s) — skipping",
            e,
        )
        return ""
...
def mark_ltm_source_rejected(
    *, ctx: HookContext, reason: str = "",
    ...
    try:
        ...
        logger.warning(
            "ATELIER_LTM_ENABLED=1 but long_term_memory not "
...
def render_sibling_evidence(
    *,
    ...
    try:
        from atelier import sibling_pool, novelty as nov_mod
        from . import tree
    except ImportError as e:
        logger.warning(
            "ATELIER_SIBLING_POOL_ENABLED=1 but sibling_pool not "
