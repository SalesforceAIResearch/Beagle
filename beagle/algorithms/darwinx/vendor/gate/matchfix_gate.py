...
lives in ``evolve.gate_hook.run_equivalence_gate``.
"""
...
def _gate_regression_tol() -> int:
    ...
    try:
        return max(0, int(os.environ.get("DARWINX_GATE_REGRESSION_TOL", "0")))
    except ValueError:
        return 0
...
def _apply_gate_tolerance(
    ...
    before the ground-truth test runs. When ``DARWINX_GATE_REGRESSION_TOL`` > 0
...
def verdict(
    *,
    ...
    try:
        ...
        # Fail-open (DARWINX_GATE_FAIL_OPEN=1): an LLM/gateway hiccup is INFRA,
        ...
        if _env_flag("DARWINX_GATE_FAIL_OPEN", False):
            _LOG.warning(
...
def verdict_with_consensus(
    *,
    ...
    # DARWINX_GATE_MAJORITY=1 we require a MAJORITY of panelists for MODIFIED
    ...
    if _env_flag("DARWINX_GATE_MAJORITY", False):
...
def aggregate_verdict(
    *,
    ...
    # gate hard-killing it. No-op unless DARWINX_GATE_REGRESSION_TOL>0.
