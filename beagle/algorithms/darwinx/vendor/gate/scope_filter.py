...
def _strip_agent_prefix(path: str) -> str:
    """Strip a leading agent-submodule prefix so patterns are relative to the agent repo.

    Diffs arrive with the path written either way depending on which repo the diff was
    taken in, so both ``<agent>/src/core/...`` and ``src/core/...`` must classify the
    same. The prefix is whatever the agent submodule is called (``monet_code`` by
    default) rather than that literal, so the allowlist keeps working when a different
    agent is evolved. Getting this wrong is silent: an unstripped prefix matches no
    internal pattern, so every path would fall through to ``outside_allowlist``.
    """
    p = path.lstrip("/")
    prefix = agent_submodule() + "/"
    if p.startswith(prefix):
        p = p[len(prefix) :]
    return p


# Historical name kept as an alias: this module is imported by report tooling that
# predates the rename, and breaking those imports would be a silent loss of scope
# reporting rather than a visible error.
_strip_monet_prefix = _strip_agent_prefix
...
def classify_path(
    ...
    *,
    ...
    rejected.
    """
    normalized = _strip_agent_prefix(path)
