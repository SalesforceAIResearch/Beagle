...
| `trajectory_loader.py` | Harbor trial dir → `verifier.TrajectoryInput` (powers `DARWINX_GATE_FITNESS_ALPHA`) | week 3 |
...
## Apples-to-apples vs the existing layers
...
`evolve.gate_hook` shim. The integration is opt-in via env vars
so default `self_evolve` behavior is unchanged:

| Env var | Default | Effect |
|---|---|---|
| `DARWINX_GATE_ENABLED` | `0` | When `1`, after each node's final eval the gate runs and a per-node decision JSON is written to `reports/<campaign>/atelier/<node_id>.json` |
| `DARWINX_GATE_FITNESS_ALPHA` | `0` | When > 0, blends pass-rate with verifier score before writing `Node.score` (affects parent selection). Live: trajectory loader walks the final-eval Harbor job dir, scores per-task with `Verifier`, blends per `FitnessRunner` |
| `DARWINX_GATE_VERIFIER_MODEL` | `gpt-5.4-mini` | Model used for verifier scoring |
| `DARWINX_GATE_VERIFIER_PROVIDER` | `sfr_gateway` | Credential profile passed to `core.credentials.resolve()` |
| `DARWINX_GATE_SCOPE_MODE` | `soft_flag` | `soft_flag` records scope violations without rejecting; `strict_reject` rejects |
| `DARWINX_GATE_REPORTS_SUBDIR` | `atelier` | Override the subdirectory under `reports/<campaign>/` where decision sidecars are written |

The hook lives in `evolve/gate_hook.py` and is
...
| `DARWINX_EVAL_CACHE_ENABLED` (env) | Skip Harbor for `(task, monet_sha, config)` combos already evaluated. Cache invalidates on commit / config / lock-file / runtime env changes. | `0` |
| `DARWINX_EVAL_CACHE_TTL_DAYS` (env) | Cache entry max age before forced miss | `14` |
| `DARWINX_EVAL_CACHE_VALIDATE_EVERY_N` (env) | Force a fresh re-eval for 1-in-N lookups (cache drift detection) | `0` (off) |
| `DARWINX_EVAL_CACHE_ROOT` (env) | Override the cache directory | `<repo>/.eval_cache` |
...
> have not been ported to this branch. The `DARWINX_EVAL_CACHE_*` knobs
...
enabling `DARWINX_GATE_FITNESS_ALPHA` in a live campaign.
