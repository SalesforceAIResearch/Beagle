...
def write_trajectory_json(
    ...
    *,
    ...
    if trajectory_format == "monet-stream-json":
        _write_monet_usage_sidecar(Path(logs_dir))
    return path


#: Full token/turn accounting the harbor-typed final_metrics/result.json can't hold
#: (they stay main-only). Name reflects contents: main-only + sub-agents + true total.
_MONET_USAGE_SIDECAR = "usage_with_subagents.json"


def _write_monet_usage_sidecar(logs_dir: Path) -> None:
    """Record the full main+sub-agent token/turn accounting so downstream can read TRUE
    totals directly. The harbor-typed ``final_metrics`` / ``result.json`` stay MAIN-ONLY;
    this sidecar carries the main-only totals, the delegated (sub-agent) split, and the
    ``main_plus_sub_agents_*`` true totals. Runtime-agnostic (written wherever
    ``write_trajectory_json`` runs: harbor, pier, raw container). Skipped when no
    sub-agents ran."""
    stream = logs_dir / _MONET_STREAM_FILENAME
    if not stream.exists():
        return
    from beagle.agents.monet._helpers import (
        count_monet_turns, parse_monet_subagent_usage, parse_monet_usage,
    )
    text = stream.read_text()
    sub_turns, sub_usage = parse_monet_subagent_usage(text)
    sub = sub_usage.to_token_counts()
    if sub_turns == 0 and not (sub.get("prompt") or sub.get("completion")):
        return  # no delegation — no sidecar
    main = parse_monet_usage(text).to_token_counts()
    main_turns = count_monet_turns(text)

    def _tot(key: str) -> int:
        return int(main.get(key) or 0) + int(sub.get(key) or 0)

    out = {
        "main_only": {
            "total_steps": main_turns,
            "total_prompt_tokens": main.get("prompt"),
            "total_cached_tokens": main.get("cache_read"),
            "total_completion_tokens": main.get("completion"),
        },
        "sub_agents": {
            "total_steps": sub_turns,
            "total_prompt_tokens": sub.get("prompt"),
            "total_cached_tokens": sub.get("cache_read"),
            "total_completion_tokens": sub.get("completion"),
        },
        "main_plus_sub_agents_total_steps": main_turns + sub_turns,
        "main_plus_sub_agents_total_prompt_tokens": _tot("prompt"),
        "main_plus_sub_agents_total_cached_tokens": _tot("cache_read"),
        "main_plus_sub_agents_total_completion_tokens": _tot("completion"),
    }
    (logs_dir / _MONET_USAGE_SIDECAR).write_text(json.dumps(out, indent=2))
